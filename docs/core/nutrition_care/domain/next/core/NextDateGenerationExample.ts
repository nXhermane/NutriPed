...
export class NextDateGenerationExample {
   ...
   */
  static async exampleDailyTreatment(
    ...
    if (treatmentRes.isSuccess) {
      ...
      console.log(
        `Traitement créé avec prochaine action: ${treatment.getNextActionDate()}`
      );
      ...
      console.log(
        `Après exécution: prochaine action ${treatment.getNextActionDate()}`
      );
   ...
   */
  static async exampleWeeklyMonitoring(
    idGenerator: GenerateUniqueId
  ): Promise<void> {
    ...
    if (parameterRes.isSuccess) {
      ...
      console.log(
        `Monitoring créé avec prochaine tâche: ${parameter.getNextTaskDate()}`
      );
      console.log(`Doit continuer: ${hasNextDate}`);
      ...
      const executionDate = DomainDateTime.now();
      const updateResult =
        NextDateUpdateService.updateMonitoringParameterAfterExecution(
          parameter,
          executionDate
        );
      console.log(
        `Après exécution: prochaine tâche ${parameter.getNextTaskDate()}`
      );
  ...
  static async exampleDailyScheduling(
    ...
    const schedule = DailyScheduleService.getDailyScheduleSummary(
      treatments,
      parameters
    );
    ...
    for (const treatment of schedule.treatmentsDue) {
      ...
      if (result.isSuccess) {
        console.log(
          `Traitement ${treatment.getCode()} exécuté. Continue: ${!result.val.treatmentCompleted}`
        );
      }
    }
    ...
    for (const parameter of schedule.monitoringParametersDue) {
      const result =
        DailyScheduleService.markMonitoringParameterAsExecuted(parameter);
      if (result.isSuccess) {
        console.log(
          `Monitoring ${parameter.getElement().code} exécuté. Continue: ${!result.val.monitoringEnded}`
        );
      }
    }
  }
