...
export class ReminderInfraMapper
  implements InfrastructureMapper<Reminder, ReminderPersistenceDto>
{
  ...
  toDomain(record: ReminderPersistenceDto): Reminder {
    const reminderRes = Reminder.create(
      { ...record, createdAt: record.reminderCreatedAt },
      record.id
    );
    if (reminderRes.isFailure)
