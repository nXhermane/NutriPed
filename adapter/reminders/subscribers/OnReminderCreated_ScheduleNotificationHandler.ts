...
export class OnReminderCreatedScheduleNotificationHandler extends EventHandler<
  ReminderCreatedEventData,
  ReminderCreatedEvent
> {
  constructor(
  ...
  async onReminderCreated(eventData: ReminderCreatedEventData): Promise<void> {
    try {
      ...
      throw new EventHandlerExecutionFailed(
        `[${OnReminderCreatedScheduleNotificationHandler.name}]: Error on reminder notification schedule. ${e} `
      );
    }
  }
}
