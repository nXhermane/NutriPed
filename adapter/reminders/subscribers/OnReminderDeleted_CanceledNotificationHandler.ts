...
@DomainEventMessage(
  "On Reminder Deleted , Cancel the reminder notification",
  true
)
export class OnReminderDeletedCanceledNotificationHandler extends EventHandler<
  ReminderDeletedEventData,
  ReminderDeletedEvent
> {
  constructor(
  ...
  async onReminderDeleted(eventData: ReminderDeletedEventData): Promise<void> {
    try {
      ...
      throw new EventHandlerExecutionFailed(
        `[${OnReminderDeletedCanceledNotificationHandler.name}]: Error on reminder notification canceling. ${e} `
      );
    }
  }
}
