import { CreateDataFieldReference, DataFieldReference } from "@/core/evaluation";
...
export class DataFieldReferenceInfraMapper implements InfrastructureMapper<DataFieldReference, DataFieldReferencePersistenceDto> {
    ...
    toDomain(record: DataFieldReferencePersistenceDto): DataFieldReference {
        const fieldRefRes = DataFieldReference.create(record as CreateDataFieldReference, record.id)
        if (fieldRefRes.isFailure) {
            throw new InfraMapToDomainError(formatError(fieldRefRes, DataFieldReference.name));
        }
