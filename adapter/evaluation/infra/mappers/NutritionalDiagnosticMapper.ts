...
export class NutritionalDiagnosticInfraMapper
  implements
    InfrastructureMapper<
      NutritionalDiagnostic,
      NutritionalDiagnosticPersistenceDto
...
{
  constructor(
    private readonly patientDiagMapper: InfrastructureMapper<
      PatientDiagnosticData,
      PatientDiagnosticDataPersistenceDto
    >,
    private readonly nutriAssessmentMapper: InfrastructureMapper<
      NutritionalAssessmentResult,
      NutritionalAssessmentResultPersistenceDto
    >
  ) {}
  toPersistence(
    ...
    return {
      ...
      result: record.result
        ? this.nutriAssessmentMapper.toPersistence(record.result)
        : undefined,
    ...
    try {
      const nutritionalDiagnostic = new NutritionalDiagnostic({
        ...
        props: {
          ...
          result: record.result
            ? this.nutriAssessmentMapper.toDomain(record.result)
            : undefined,
          patientId: record.patientId,
          modificationHistories: record.modificationHistories.map(modifHis => {
            return new DiagnosticModification({
              date: new DomainDate(modifHis.date),
              nextResult: this.nutriAssessmentMapper.toDomain(
                modifHis.nextResult
              ),
              prevResult: this.nutriAssessmentMapper.toDomain(
                modifHis.prevResult
              ),
      ...
      throw new InfraMapToDomainError(
        formatError(e as never, NutritionalDiagnosticInfraMapper.name)
      );
    }
  }
}
