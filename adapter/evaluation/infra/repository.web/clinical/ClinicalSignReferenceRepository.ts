  ...
  implements ClinicalSignReferenceRepository
{
  ...
  async getByCode(code: SystemCode): Promise<ClinicalSignReference> {
    try {
      ...
      return new Promise((resolve, reject) => {
        ...
        request.onsuccess = event => {
          ...
          if (!result) {
            reject(
              new Error(
                `ClinicalSignReference with code ${code.unpack()} not found`
              )
            );
            return;
          }
          resolve(
            this.mapper.toDomain(result as ClinicalSignReferencePersistenceDto)
          );
        };

        request.onerror = event => {
          console.error(
            "Error fetching clinical sign reference by code:",
            event
          );
      ...
      console.error(error);
      throw new Error(
        `Failed to get clinical sign reference by code: ${error}`
      );
    }
  }
      ...
      return new Promise((resolve, reject) => {
        ...
        request.onsuccess = event => {
          ...
          const codes = results.map(
            (dto: ClinicalSignReferencePersistenceDto) =>
              SystemCode.create(dto.code).val
          );
          resolve(codes);
        };
