      ...
      return new Promise((resolve, reject) => {
        ...
        request.onsuccess = event => {
          ...
          if (!result) {
            reject(
              new Error(
                `AnthropometricMeasure with code ${code.unpack()} not found`
              )
            );
            return;
          }
          resolve(
            this.mapper.toDomain(result as AnthropometricMeasurePersistenceDto)
          );
        };

        request.onerror = event => {
          console.error(
            "Error fetching anthropometric measure by code:",
            event
          );
      ...
      return new Promise((resolve, reject) => {
        ...
        request.onsuccess = event => {
          ...
          const codes = results.map(
            (code: string) => SystemCode.create(code).val
          );
          resolve(codes);
        };
