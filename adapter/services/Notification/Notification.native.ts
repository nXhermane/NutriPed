  ...
  implements IReminderNotificationService
{
  ...
  async scheduleNotification(
    ...
    await Notifications.scheduleNotificationAsync({
      content: {
        ...
        badge: 1,
        categoryIdentifier: notification.hasAction
          ? NOTIFICATION_CONFIG.CATEGORIES.REMINDER_ACTIONS.id
          : NOTIFICATION_CONFIG.CATEGORIES.NO_ACTION.id,
        data: {
  ...
  async cancelNotfication(reminderId: AggregateID): Promise<void> {
    const notificationIdMap =
      await this.getScheduledRemiderIdAssociatedWithNotificationId();
  ...
  private getAllScheduledNotifications(): Promise<
    Notifications.NotificationRequest[]
  > {
    return Notifications.getAllScheduledNotificationsAsync();
  }
  private async getScheduledRemiderIdAssociatedWithNotificationId(): Promise<
    Map<AggregateID, { notificationId: string }>
  > {
    ...
    for (const notification of scheduledNotifications) {
      ...
      reminderIdMap.set(reminderId as AggregateID, {
  ...
  private mapTrigger(
    ...
    switch (trigger.type) {
      ...
      case ReminderTriggerType.RECURRING:
        ...
        if (
          trigger.frequency === RECURRING_FREQUENCY.WEEKLY &&
          trigger.daysOfWeek
        ) {
          const [hour, minute] = trigger.time.split(":").map(Number);
          return {
