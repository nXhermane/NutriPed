...
export class DailyCareJournalInfraMapper
  implements InfrastructureMapper<DailyCareJournal, DailyJournalPersistenceDto>
{
  ...
  toDomain(record: DailyJournalPersistenceDto): DailyCareJournal {
    ...
    const monitoringValuesRes = record.monitoringValues.map(
      MonitoringEntry.create
    );
    const observationRes = record.observations.map(ClinicalEvent.create);
    const actions = record.treatmentActions.map(
      NutritionalTreatmentAction.create
    );
    const appetiteTestsRes = record.appetiteTestResults.map(
      AppetiteTestResult.create
    );
    ...
    if (combinedRes.isFailure)
      throw new InfraMapToDomainError(
        formatError(combinedRes, DailyCareJournalInfraMapper.name)
      );
