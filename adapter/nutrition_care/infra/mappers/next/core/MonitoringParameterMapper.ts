...
import { MonitoringParameter, CreateMonitoringParameter } from "@/core/nutrition_care/domain/next/core/models/entities";
import { MonitoringParameterPersistenceDto } from "../../../dtos/next/core";

export class MonitoringParameterInfraMapper
    implements InfrastructureMapper<MonitoringParameter, MonitoringParameterPersistenceDto> {
    toPersistence(entity: MonitoringParameter): MonitoringParameterPersistenceDto {
    ...
    toDomain(record: MonitoringParameterPersistenceDto): MonitoringParameter {
        ...
        if (result.isFailure) {
            throw new Error(`Failed to create MonitoringParameter from persistence data: ${result.err}`);
        }
