...
export class DailyCareRecordInfraMapper
  implements
    InfrastructureMapper<DailyCareRecord, DailyCareRecordPersistenceDto>
{
  ...
  toPersistence(entity: DailyCareRecord): DailyCareRecordPersistenceDto {
    return {
      ...
      treatmentActions: entity.props.treatmentActions.map(action =>
        this.actionMapper.toPersistence(action)
      ),
      monitoringTasks: entity.props.monitoringTasks.map(task =>
        this.taskMapper.toPersistence(task)
      ),
