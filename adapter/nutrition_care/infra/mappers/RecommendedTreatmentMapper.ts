...
export class RecommendedTreatmentInfraMapper {
    ...
    return {
      ...
      applicabilyCondition: {
        ...
        variablesExplanation:
          entity.getApplicabilyCondition().variablesExplanation,
      },
      code: entity.getCode(),
  ...
  toDomain(record: RecommendedTreatmentPersistenceDto): RecommendedTreatment {
    const recommendedTreatmentRes = RecommendedTreatment.create(
      record,
      record.id
    );
    if (recommendedTreatmentRes.isFailure) {
      throw new InfraMapToDomainError(
        formatError(
          recommendedTreatmentRes,
          RecommendedTreatmentInfraMapper.name
        )
      );
    }
    const { id, createdAt, updatedAt, ...props } =
      recommendedTreatmentRes.val.getProps();
