...
export class HumanDateFormatter {
  ...
  static toRelativeDate(
    ...
    try {
      ...
      throw new Error(
        `Erreur lors du formatage de la date relative : ${error}`
      );
    }
  }
  ...
  static toFollowUpDate(sinceDateString: string): string {
    try {
      ...
      if (sinceDate > now) {
        throw new Error(
          "La date de début de suivi ne peut pas être dans le futur"
        );
      }
      ...
      throw new Error(
        `Erreur lors du formatage de la date de suivi : ${error}`
      );
    }
  }
}
