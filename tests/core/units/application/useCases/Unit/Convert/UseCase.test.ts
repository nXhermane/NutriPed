...
import { IUnitConverterService, Unit, UnitRepository, UnitType } from '../../../../../../../core/units/domain';
import { Result } from '../../../../../../../core/shared';

// Mock Unit entities
const kgUnit = Unit.create({ name: 'Kilogram', code: 'kg', conversionFactor: 1000, baseUnitCode: 'g', type: UnitType.MASS }, 'kg-id').val;
const gUnit = Unit.create({ name: 'Gram', code: 'g', conversionFactor: 1, baseUnitCode: 'g', type: UnitType.MASS }, 'g-id').val;
...
describe('ConvertUnitUseCase', () => {
  ...
  it('should convert between two units successfully', async () => {
    ...
    expect(mockUnitRepository.getByCode).toHaveBeenCalledWith(expect.objectContaining({ props: { _value: 'kg' } }));
    expect(mockUnitRepository.getByCode).toHaveBeenCalledWith(expect.objectContaining({ props: { _value: 'g' } }));
    expect(mockConversionService.convert).toHaveBeenCalledWith(1, kgUnit, gUnit);
  });
  ...
  it('should return a failure when units are incompatible', async () => {
    // Arrange
    mockConversionService.convert.mockReturnValue(Result.fail('Incompatible units'));
  ...
  it('should return a failure when a unit is not found', async () => {
    // Act
    const result = await useCase.execute({ value: 1, from: 'non-existent', to: 'g' });
