...
describe('UnitConverterService', () => {
  ...
  const gramResult = Unit.create({ name: 'Gram', code: 'g', conversionFactor: 1, baseUnitCode: 'g', type: UnitType.MASS }, 'g-id');
  const kilogramResult = Unit.create({ name: 'Kilogram', code: 'kg', conversionFactor: 1000, baseUnitCode: 'g', type: UnitType.MASS }, 'kg-id');
  const meterResult = Unit.create({ name: 'Meter', code: 'm', conversionFactor: 1, baseUnitCode: 'm', type: UnitType.LENGTH }, 'm-id');

  if (gramResult.isFailure || kilogramResult.isFailure || meterResult.isFailure) {
  ...
  it('should return a failure result when converting between incompatible units', () => {
    ...
    expect(result.error).toBe("It's not possible to convert a unit with different base unit.");
  });
