import { CalculateGrowthIndicatorValueUseCase } from '../../../../../../../../../core/evaluation/application/useCases/anthropometric/GrowthIndicatorValue/CalculateGrowthIndicatorValue/UseCase';
import { CalculateGrowthIndicatorValueRequest } from '../../../../../../../../../core/evaluation/application/useCases/anthropometric/GrowthIndicatorValue/CalculateGrowthIndicatorValue/Request';
...
import { GrowthIndicatorValue } from '../../../../../../../../../core/evaluation/domain';
...
describe('CalculateGrowthIndicatorValueUseCase', () => {
  ...
  describe('execute', () => {
    it('should calculate growth indicator value successfully', async () => {
      ...
      const validationResult = MockFactory.createSuccessResult({ isValid: true, errors: [] });
      mockAnthropometricValidationService.validate.mockResolvedValue(validationResult);

      const anthropometricVariables = { weight: 10, height: 100 };
      const anthropometricVariablesResult = MockFactory.createSuccessResult(anthropometricVariables);
      ...
      const growthIndicatorValueResult = MockFactory.createSuccessResult(growthIndicatorValue);
      mockGrowthIndicatorService.calculateIndicator.mockResolvedValue(growthIndicatorValueResult);
      ...
      expect(mockAnthropometricVariablesService.generate).toHaveBeenCalledTimes(2);
      expect(mockGrowthIndicatorService.calculateIndicator).toHaveBeenCalledWith(
    ...
    it('should return left when anthropometric data creation fails', async () => {
      ...
      expect(mockAnthropometricValidationService.validate).not.toHaveBeenCalled();
      expect(mockAnthropometricVariablesService.generate).not.toHaveBeenCalled();
      expect(mockGrowthIndicatorService.calculateIndicator).not.toHaveBeenCalled();
    });

    it('should return left when validation fails', async () => {
      // Arrange
      const validationResult = MockFactory.createFailureResult('Validation failed');
      mockAnthropometricValidationService.validate.mockResolvedValue(validationResult);
    ...
    it('should return left when generating anthropometric variables fails', async () => {
      ...
      const anthropometricVariablesResult = MockFactory.createFailureResult('Failed to generate variables');
      mockAnthropometricVariablesService.generate.mockResolvedValue(anthropometricVariablesResult);
    ...
    it('should return left when calculating growth indicator value fails', async () => {
      ...
      const growthIndicatorValueResult = MockFactory.createFailureResult('Failed to calculate indicator');
      mockGrowthIndicatorService.calculateIndicator.mockResolvedValue(growthIndicatorValueResult);
    ...
    it('should return left when generating anthropometric variables with indicator value fails', async () => {
      ...
      mockAnthropometricVariablesService.generate
        .mockResolvedValueOnce(anthropometricVariablesResult)
        .mockResolvedValueOnce(MockFactory.createFailureResult('Failed to generate variables with indicator'));
