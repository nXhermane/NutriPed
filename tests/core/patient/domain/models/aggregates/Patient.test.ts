import { Patient, CreatePatientProps } from '@patient/domain/models/aggregates/Patient';
import { AggregateID, Birthday, FullName, Gender, Sex, Contact, Address } from '@shared';
...
describe('Patient', () => {
  ...
  const createValidPatient = (): Patient => {
    ...
    if (patientResult.isFailure) {
      throw new Error(`Failed to create a valid patient for testing: ${patientResult.err}`);
    }
  ...
  describe('entity methods', () => {
    ...
    it('should change the patient birthday', () => {
      ...
      const newBirthday = Birthday.create(newBirthdayDate.toISOString().split('T')[0]).val;
      patient.changeBirthday(newBirthday);
      expect(patient.getBirthday()).toBe(newBirthdayDate.toISOString().split('T')[0]);
    });
    ...
    it('should throw an error when changing to a birthday that exceeds the max age', () => {
      ...
      const invalidBirthday = new Date();
      invalidBirthday.setFullYear(invalidBirthday.getFullYear() - 20);
      const newBirthday = Birthday.create(invalidBirthday.toISOString().split('T')[0]).val;
