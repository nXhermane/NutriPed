import {
  ...
  AppetiteTestAppServiceUseCases,
} from "@/core/nutrition_care/application/services/AppetiteTestService";
import {
...
describe("AppetiteTestAppService", () => {
  ...
  describe("evaluateAppetite", () => {
    it("should return the appetite test result on successful evaluation", async () => {
      // Arrange
      const request: EvaluateAppetiteRequest = {
        appetiteTestId: "test-id",
        patientId: "patient-id",
      };
      const response = { result: "pass" };
      mockUseCases.evaluateAppetiteUC.execute.mockResolvedValue(
      ...
      // Assert
      expect(result).toEqual({ data: response });
      expect(mockUseCases.evaluateAppetiteUC.execute).toHaveBeenCalledWith(
        request
      );
    });
