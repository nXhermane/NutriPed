import {
  MedicineAppService,
  MedicineServiceUseCases,
} from "@/core/nutrition_care/application/services/MedicineService";
import {
...
describe("MedicineAppService", () => {
  ...
  describe("getDosage", () => {
    it("should return the medicine dosage on successful retrieval", async () => {
      // Arrange
      const request: GetMedicineDosageRequest = {
        medicineId: "test-id",
        patientId: "patient-id",
      };
      const response = { dosage: "10ml" };
      mockUseCases.getDosageUC.execute.mockResolvedValue(
