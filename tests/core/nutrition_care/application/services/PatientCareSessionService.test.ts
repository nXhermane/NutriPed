import {
  ...
  PatientCareSessionServiceUseCases,
} from "@/core/nutrition_care/application/services/PatientCareSessionService";
import {
...
describe("PatientCareSessionAppService", () => {
  ...
  describe("create", () => {
    it("should return the id on successful creation", async () => {
      const request: CreatePatientCareSessionRequest = {
  ...
  describe("get", () => {
    it("should return the patient care session on successful retrieval", async () => {
      ...
      const response = { id: "test-id" };
      mockUseCases.getUC.execute.mockResolvedValue(
  ...
  describe("addData", () => {
    it("should return void on successful data addition", async () => {
      const request: AddDataToPatientCareSessionRequest = {
        patientId: "patient-id",
        data: {},
      };
      mockUseCases.addDataUC.execute.mockResolvedValue(
        right(Result.ok(undefined))
      );
  ...
  describe("evaluatePatientAppetite", () => {
    it("should return appetite test result on successful evaluation", async () => {
      const request: EvaluatePatientAppetiteRequest = {
        patientId: "patient-id",
        appetiteTestId: "test-id",
      };
      const response = { result: "pass" };
      mockUseCases.evaluatePatientAppetiteUC.execute.mockResolvedValue(
        right(Result.ok(response as any))
      );
      const result = await service.evaluatePatientAppetite(request);
      expect(result).toEqual({ data: response });
      expect(
        mockUseCases.evaluatePatientAppetiteUC.execute
  ...
  describe("orientPatient", () => {
    it("should return orientation result on successful orientation", async () => {
      ...
      mockUseCases.orientPatientUC.execute.mockResolvedValue(
        right(Result.ok(response as any))
      );
      const result = await service.orientPatient(request);
      expect(result).toEqual({ data: response });
      expect(mockUseCases.orientPatientUC.execute).toHaveBeenCalledWith(
        request
      );
    });
  ...
  describe("makeCareSessionReady", () => {
    it("should return true on successful status change", async () => {
      const request: MakePatientCareSessionReadyRequest = {
        patientId: "patient-id",
      };
      const response = true;
      mockUseCases.makeCareSessionReadyUC.execute.mockResolvedValue(
        right(Result.ok(response))
      );
      const result = await service.makeCareSessionReady(request);
      expect(result).toEqual({ data: response });
      expect(mockUseCases.makeCareSessionReadyUC.execute).toHaveBeenCalledWith(
        request
      );
    });
  ...
  describe("getDailyJournals", () => {
    it("should return daily journals on successful retrieval", async () => {
      ...
      mockUseCases.getDailyJournalsUC.execute.mockResolvedValue(
        right(Result.ok(response as any))
      );
      const result = await service.getDailyJournals(request);
      expect(result).toEqual({ data: response });
      expect(mockUseCases.getDailyJournalsUC.execute).toHaveBeenCalledWith(
        request
      );
    });
