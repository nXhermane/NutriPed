import { PatientCareSession, IPatientCareSession, PatientCareSessionStatus } from "@/core/nutrition_care/domain/core/models/aggregates/PatientCareSession";
import { DailyCareJournal } from "@/core/nutrition_care/domain/core/models/entities/DailyCareJournal";
import { CreateEntityProps, EntityUniqueID, DomainDate, DateManager } from "@shared";

describe("PatientCareSession", () => {
    const createPatientCareSession = (props: Partial<IPatientCareSession>): PatientCareSession => {
    ...
    it("should throw an error if the daily journal is not for today", () => {
        ...
        const dailyJournal = DailyCareJournal.create(yesterdayStr, "journal-id").val;
        ...
        // Act & Assert
        expect(() => patientCareSession.addDailyJournal(dailyJournal)).toThrow("The added journal is not of today.Please only the day journal can't be added.");
    });
    ...
    it("should end the care session", () => {
        ...
        // Assert
        expect(patientCareSession.getProps().status).toBe(PatientCareSessionStatus.COMPLETED);
