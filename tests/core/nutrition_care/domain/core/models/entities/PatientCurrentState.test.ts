import { PatientCurrentState, CreateCurrentStateProps } from "@/core/nutrition_care/domain/core/models/entities/PatientCurrentState";
import { AnthroSystemCodes, CLINICAL_SIGNS, BIOCHEMICAL_REF_CODES, TREATMENT_HISTORY_VARIABLES_CODES, COMPLICATION_CODES } from "@/core/constants";
import { APPETITE_TEST_CODES, APPETITE_TEST_RESULT_CODES } from "@/core/nutrition_care/domain/modules";
...
describe("PatientCurrentState", () => {
    const createProps: CreateCurrentStateProps = {
        ...
        otherData: {
            [TREATMENT_HISTORY_VARIABLES_CODES.PREVIOUS_TREATMENT]: "ORIENTATION_HOME",
        },
    };
    ...
    it("should create a new patient current state", () => {
        ...
        expect(state.getAnthroVariables()).toEqual({ [AnthroSystemCodes.WEIGHT]: 10 });
    });
