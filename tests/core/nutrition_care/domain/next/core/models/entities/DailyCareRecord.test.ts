...
} from "@/core/nutrition_care/domain/next/core/models/entities/DailyCareRecord";
...
jest.mock(
  "@/core/nutrition_care/domain/next/core/models/entities/DailyCareAction"
);
jest.mock(
  "@/core/nutrition_care/domain/next/core/models/entities/DailyMonitoringTask"
);

describe("DailyCareRecord", () => {
  beforeEach(() => {
    (DailyCareAction.create as jest.Mock).mockReturnValue(
      Result.ok({ isCompleted: () => true } as DailyCareAction)
    );
    (DailyMonitoringTask.create as jest.Mock).mockReturnValue(
      Result.ok({ isCompleted: () => true } as DailyMonitoringTask)
    );
  });
  ...
  it("should not be completed if some actions are not completed", () => {
    // Arrange
    (DailyCareAction.create as jest.Mock).mockReturnValue(
      Result.ok({ isCompleted: () => false } as DailyCareAction)
    );
    const record = DailyCareRecord.create(
      { ...createProps, treatmentActions: [{} as any] },
      "record-id"
    ).val;
