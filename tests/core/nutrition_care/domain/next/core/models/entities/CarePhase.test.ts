import { CarePhase, CreateCarePhase, CarePhaseStatus } from "@/core/nutrition_care/domain/next/core/models/entities/CarePhase";
...
jest.mock("@/core/nutrition_care/domain/next/core/models/entities/MonitoringParameter");
jest.mock("@/core/nutrition_care/domain/next/core/models/entities/OnGoingTreatment");

describe("CarePhase", () => {
    beforeEach(() => {
        (MonitoringParameter.create as jest.Mock).mockReturnValue(Result.ok({} as MonitoringParameter));
        (OnGoingTreatment.create as jest.Mock).mockReturnValue(Result.ok({} as OnGoingTreatment));
    });
