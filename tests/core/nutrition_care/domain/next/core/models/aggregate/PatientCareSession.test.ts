import { PatientCareSession, PatientCareSessionStatus } from "@/core/nutrition_care/domain/next/core/models/aggregate/PatientCareSession";
...
jest.mock("@/core/nutrition_care/domain/next/core/models/entities/DailyCareRecord");

describe("PatientCareSession", () => {
    beforeEach(() => {
        (CarePhase.create as jest.Mock).mockReturnValue(Result.ok({} as CarePhase));
        (DailyCareRecord.create as jest.Mock).mockReturnValue(Result.ok({ getDate: () => DomainDateTime.now().toString() } as DailyCareRecord));
    });
    ...
    it("should transition to a new phase", () => {
        // Arrange
        const session = PatientCareSession.create({ patientId: "patient-id" }, "session-id").val;
    ...
    it("should update the current daily record", () => {
        // Arrange
        const session = PatientCareSession.create({ patientId: "patient-id" }, "session-id").val;
        const record = { getDate: () => DomainDateTime.now().toString() } as DailyCareRecord;
