...
} from "@/core/nutrition_care/domain/next/core/services/interfaces";
...
describe("CarePhaseManagerService", () => {
  ...
  beforeEach(() => {
    ...
    service = new CarePhaseManagerService(
      idGenerator,
      carePhaseRefService,
      careSessionVariableGenerator,
      carePlanApplicator
  ...
  it("should generate a care phase", async () => {
    ...
    careSessionVariableGenerator.generateIntialVariables.mockResolvedValue(
      Result.ok({})
    );
    carePhaseRefService.determineApplicableCare.mockResolvedValue(
      Result.ok({} as any)
    );
