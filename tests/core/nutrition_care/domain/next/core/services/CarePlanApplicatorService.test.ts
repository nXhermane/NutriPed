...
import { ITreatmentManager, IMonitoringParameterManager, ITriggerExecutor } from "@/core/nutrition_care/domain/next/core/services/helpers";
...
describe("CarePlanApplicatorService", () => {
    ...
    beforeEach(() => {
        ...
        service = new CarePlanApplicatorService(treatmentManager, monitoringParameterManager, triggerExecutor);
    });

    it("should apply a care plan", async () => {
        // Arrange
        const recommendation = { applicableTreatments: [], monitoringElements: [] } as any;
        const targetCarePhase = { getProps: () => ({ onGoingTreatments: [], monitoringParameters: [] }) } as any;
        treatmentManager.synchronizeTreatments.mockReturnValue(Result.ok({ newTreatments: [], stoppedTreatments: [] }) as any);
        monitoringParameterManager.synchronizeMonitoringParameters.mockReturnValue(Result.ok({}) as any);
        triggerExecutor.executeAllTriggers.mockResolvedValue(Result.ok({ executedTriggers: { onStart: 0, onEnd: 0 }, errors: [] }) as any);
        ...
        expect(monitoringParameterManager.synchronizeMonitoringParameters).toHaveBeenCalled();
