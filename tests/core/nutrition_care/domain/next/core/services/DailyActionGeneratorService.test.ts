...
describe("DailyActionGeneratorService", () => {
  ...
  beforeEach(() => {
    ...
    service = new DailyActionGeneratorService(
      idGenerator,
      medicineDosageService,
      nutritionalProductService
    );
    (DailyCareAction.create as jest.Mock).mockReturnValue(
      Result.ok({} as DailyCareAction)
    );
  });
  ...
  it("should generate daily actions", async () => {
    ...
    const treatment = {
      getProps: () => ({
        recommendation: {
          getType: () => RECOMMENDED_TREATMENT_TYPE.NUTRITIONAL,
          unpack: () => ({ code: "f75" }),
          getAdjustmentPercentage: () => 0,
        },
      }),
    } as any;
    nutritionalProductService.getDosage.mockResolvedValue(
      Result.ok({
