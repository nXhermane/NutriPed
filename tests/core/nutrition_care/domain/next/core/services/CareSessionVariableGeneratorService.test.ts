...
import { IComputedVariablePerformerACL, MedicalRecordVariableTransformerAcl } from "@/core/nutrition_care/domain/next/core/ports";
...
describe("CareSessionVariableGeneratorService", () => {
    ...
    beforeEach(() => {
        ...
        service = new CareSessionVariableGeneratorService(computedVariablePerformerAcl, medicalRecordVariableTransformerAcl);
    });
    ...
    it("should generate initial variables", async () => {
        ...
        medicalRecordVariableTransformerAcl.getVariableByDate.mockResolvedValue(Result.ok({ "var1": 1 }));
        ...
        expect(medicalRecordVariableTransformerAcl.getVariableByDate).toHaveBeenCalledWith(patientId, targetDate);
    });
});
