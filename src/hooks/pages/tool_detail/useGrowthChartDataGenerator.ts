import { ChartDataDto, DAY_IN_MONTHS, DAY_IN_YEARS, GrowthRefChartAndTableCodes, GrowthReferenceChartDto } from "@/core/diagnostics";
import { CHART_UI_DATA, DisplayMode, LengthHeightMode } from "@/src/constants/ui";
...
export type ChartDataXAxisProps = ChartDataXAxisPropsAgeBased | ChartDataXAxisPropsLenHeiBased
...
export function useGrowthChartDataGenerator(chartData: ChartDataDto[], code: GrowthRefChartAndTableCodes) {
    const [chartUiData, setChartUiData] = useState<typeof CHART_UI_DATA[keyof typeof CHART_UI_DATA]>()
    const [displayMode, setDisplayMode] = useState<DisplayMode>()
    const [lenHeiInterval, setLenHeiInterval] = useState<number>(10)
    const [dayInterval, setDayInterval] = useState<number>()
    const [displayedXAxisRange, setDisplayedXAxisRange] = useState<[ChartDataXAxisProps, ChartDataXAxisProps] | "all">("all")
    useEffect(() => {
        if (code) {
    ...
    const generateData = useMemo(() => {
        if (chartUiData && chartData) {
            if (LengthHeightMode.includes(chartUiData.defaultDisplayMode as typeof LengthHeightMode[number])) {
                ...
                if (!displayMode && !chartUiData.availableDisplayMode.includes(displayMode as never)) return []
                if (dayInterval) {
                    const processedData: ChartDataPoint[] = []
                    for (let index = 0; index <= chartData.length - 1; index++) {
                        const { curvePoints, value } = chartData[index]
        ...
        return []
    }, [chartUiData, chartData, lenHeiInterval, displayMode, dayInterval, displayedXAxisRange])


    return { data: generateData, setLenHeiInterval, setDisplayedXAxisRange, setDisplayMode, setDayInterval, displayMode, displayedXAxisRange, chartUiData }
}