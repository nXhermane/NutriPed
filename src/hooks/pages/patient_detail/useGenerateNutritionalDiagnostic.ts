...
import { GLOBAL_DIAGNOSTIC_UI_INDICATOR, PATIENT_STATE } from "@/src/constants/ui";
...
export function useGenerateNutritionalDiagnostic() {
    ...
    const generate = useCallback(async () => {
        ...
        if ('data' in result) {
            ...
            const findedIndex = patientInteractionLists.findIndex((interact) => interact.patientId === patient.id)
            if (findedIndex != -1) {
                const interaction = patientInteractionLists[findedIndex]
                const isAttention = result.data.globalDiagnostics.some(diagnostic => ["Sévère", "Modérée"].includes(GLOBAL_DIAGNOSTIC_UI_INDICATOR[diagnostic.code as keyof typeof GLOBAL_DIAGNOSTIC_UI_INDICATOR].label))
                const isNormal = (result.data.globalDiagnostics.length == 0 && result.data.growthIndicatorValues.length != 0) || result.data.globalDiagnostics.every(diagnostic => ["Normal", "Risque"].includes(GLOBAL_DIAGNOSTIC_UI_INDICATOR[diagnostic.code as keyof typeof GLOBAL_DIAGNOSTIC_UI_INDICATOR].label))
                if (isAttention || isNormal) {
                    dispatch(recordInteraction({ ...interaction, state: isAttention ? PATIENT_STATE.ATTENTION : isNormal ? PATIENT_STATE.NORMAL : PATIENT_STATE.NEW }))
                }
            }
        } else {
