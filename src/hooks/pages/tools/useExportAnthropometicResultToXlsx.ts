...
export function useExportAnthropometicResultToXlsx() {
    const mapDataToRow = useMemo(() => (data: AnthropometricCalculatorResultDataType, headers: string[]): string[] => {
        const rawData = {
            ...
            ...data.usedData.anthropometricData.reduce((acc, curVal) => ({ ...acc, [curVal.code]: `${curVal.value} ${curVal.unit}` }), {}),
            ...data.result.reduce((acc, curVal) => ({ ...acc, [curVal.code]: `${curVal.value > 4 ? ">4" : curVal.value < -4 ? "<-4" : curVal.value}` }), {})
        }
        const row: string[] = []
        headers.forEach((key) => {
    ...
    const applyTemplateStyles = useMemo(() => (newWorksheet: XLSX.WorkSheet, templateWorksheet: XLSX.WorkSheet) => {
