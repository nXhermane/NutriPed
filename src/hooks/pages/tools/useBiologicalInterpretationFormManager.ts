...
import { BirthDateField, BirthDateToTodayZodSchema, SexField, SexZodSchema, validateWithSchemaPipeline } from "@/src/constants/ui";
import { useEffect, useMemo, useState } from "react";
import { z } from "zod";

export function useBiologicalInterpretationFormManager(availableBioRefs: BiochemicalReferenceDto[] = []) {
    ...
    const formSchema = useMemo<FormSchema>(() => {
        return [{ section: "Informations généraux", fields: [BirthDateField, SexField] }, { section: "Marqueurs sélectionnées", fields: fieldList }]
    }, [fieldList])
    const zodValidation = useMemo<DynamicFormZodSchemaType>(() => {
        return {
            validate(data) {
                return validateWithSchemaPipeline(data, [BirthDateToTodayZodSchema, SexZodSchema, ...zodSchemaList])
            },
        }
    }, [zodSchemaList])
    useEffect(() => {
        if (selectedBioMarker.length != 0) {
            ...
            for (const biomarkerId of selectedBioMarker) {
                const findedBiomarker = availableBioRefs.find(ref => ref.id === biomarkerId)
                if (findedBiomarker) {
                    const field: IField = {
                        type: 'quantity',
