...
export function useClinicalSignReferenceFormGenerator(
  clinicalSignRefs?: ClinicalSignReferenceDto[],
  withGeneralInfo: boolean = true
) {
  const [clinicalSignRefFormData, setClinicalSignFormData] = useState<{
    schema: FormSchema;
    zodSchema: DynamicFormZodSchemaType;
  } | null>(null);
  const [variableUsageMap, setVariableUsageMap] =
    useState<VariableUsageMap | null>(null);
  const [onLoading, setOnLoading] = useState<boolean>(false);
  const generateFormClinicalSignsData = useMemo(
    () =>
      (
        data: ClinicalSignReferenceDto[]
      ): { schema: FormSchema; zodSchema: DynamicFormZodSchemaType } => {
        ...
        for (const struture of Object.values(form.formStructure)) {
          const { fields, section, zodSchemas } =
            processor.generateDynamicFormSection(struture);
            ...
            validate(data: any) {
              return validateWithSchemaPipeline(data, [
                ...(withGeneralInfo
                  ...
                  : []),
                ...dynamicFromValidationZodSchema,
              ]);
            },
          },
        };
...
type UniqueVariables = {
  [dataCode: string]: ClinicalSignDataDto & {
    usedInSigns: string[];
    category: ClinicalSignRefDataCategory;
  };
};
...
class ClinicalDataProcessor {
  ...
  private categorizeVariable(code: string): ClinicalSignRefDataCategory {
    for (const [category, { prefixs }] of Object.entries(
      ClinicalRefCategoryUiData
    )) {
      for (const prefix of prefixs) {
        if (code.startsWith(prefix))
          return category as ClinicalSignRefDataCategory;
      }
    }
    ...
    Object.entries(this.uniqueVariables).forEach(([code, variable]) => {
      ...
      if (!grouped[category]) {
        grouped[category] = {
          title:
            ClinicalRefCategoryUiData[
              category as keyof typeof ClinicalRefCategoryUiData
            ].uiText,
          priority:
            ClinicalRefCategoryUiData[
              category as keyof typeof ClinicalRefCategoryUiData
    ...
    switch (variable.dataType) {
      ...
      case ClinicalDataType.RANGE: {
        ...
        const zodSchema = z.object({
          [variable.code]: z
            .number()
            .min(variable.dataRange?.[0]!)
            .max(variable.dataRange?.[1]!)
            .default(variable.dataRange?.[0]!),
        });
        return {
          field,
      ...
      default: {
        console.warn(
          "Unsupported Clinical Data Type Detected!",
          variable.dataType
        );
        return null;
      }
