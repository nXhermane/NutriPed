...
import { ClinicalSignReferenceDto, GetClinicalSignReferenceRequest } from "@/core/diagnostics";
import { useEffect, useState } from "react";

export function useClinicalReference(req?: GetClinicalSignReferenceRequest) {
    const { diagnosticServices: { clinicalSign } } = usePediatricApp()
    const [clinicalSignRefList, setClinicalSignRefList] = useState<ClinicalSignReferenceDto[]>([])
