...
export class UpdateReminderUseCase
  implements UseCase<UpdateReminderRequest, UpdateReminderResponse>
{
  ...
  private updateReminder(
    reminder: Reminder,
    data: UpdateReminderRequest["data"]
  ): Result<void> {
    try {
      ...
      if (data.trigger) {
        ...
        if (triggerResult.isFailure)
          return Result.fail(
            formatError(triggerResult, UpdateReminderUseCase.name)
          );
      ...
      if (data.actions) {
        const actionsResults = data.actions.map(action =>
          ReminderAction.create(action)
        );
        const combinedResult = Result.combine(actionsResults);
        if (combinedResult.isFailure)
          return Result.fail(
            formatError(combinedResult, UpdateReminderUseCase.name)
          );
