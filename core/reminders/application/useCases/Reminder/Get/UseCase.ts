...
export class GetReminderUseCase
  implements UseCase<GetReminderRequest, GetReminderResponse>
{
  constructor(
    private readonly repo: ReminderRepository,
    private readonly mapper: ApplicationMapper<Reminder, ReminderDto>
  ) {}
  async execute(request: GetReminderRequest): Promise<GetReminderResponse> {
    try {
      ...
      if (reminders.length === 0)
        return left(Result.fail("No reminders found"));
