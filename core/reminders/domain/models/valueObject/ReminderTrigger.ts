...
export type IReminderTriggerProps =
  | IntervalTrigger
  | DateTimeTrigger
  | RecurringTrigger;
...
export class ReminderTrigger extends ValueObject<IReminderTriggerProps> {
  protected validate(props: Readonly<IReminderTriggerProps>): void {
    if (Guard.isEmpty(props.data).succeeded) {
      throw new ArgumentNotProvidedException(
        "Reminder trigger data cannot be empty."
      );
    }
    ...
    switch (props.type) {
      case ReminderTriggerType.INTERVAL:
        if (!("every" in props.data)) {
          throw new ArgumentNotProvidedException(
            "Missing 'every' in INTERVAL trigger."
          );
        }
        break;
      ...
      case ReminderTriggerType.DATE_TIME:
        if (!("scheduled" in props.data)) {
          throw new ArgumentNotProvidedException(
            "Missing 'scheduled' in DATE_TIME trigger."
          );
        }
        break;
      ...
      case ReminderTriggerType.RECURRING:
        ...
        if (!frequency || !time) {
          throw new ArgumentNotProvidedException(
            "Missing 'frequency' or 'time' in RECURRING trigger."
          );
        }
        if (frequency === RECURRING_FREQUENCY.WEEKLY && !daysOfWeek) {
          throw new ArgumentNotProvidedException(
            "'daysOfWeek' is required for WEEKLY triggers."
          );
        }
        break;
    }
  ...
  static create(props: CreateReminderTriggerProps): Result<ReminderTrigger> {
    try {
      ...
      switch (props.type) {
        case ReminderTriggerType.INTERVAL: {
          const { every } =
            props.data as ReminderTriggerInputData[ReminderTriggerType.INTERVAL];
        ...
        case ReminderTriggerType.DATE_TIME: {
          const { scheduled } =
            props.data as ReminderTriggerInputData[ReminderTriggerType.DATE_TIME];
        ...
        case ReminderTriggerType.RECURRING: {
          const { frequency, time, daysOfWeek } =
            props.data as ReminderTriggerInputData[ReminderTriggerType.RECURRING];
      ...
      return Result.ok(
        new ReminderTrigger({ type: props.type, data } as IReminderTriggerProps)
      );
    } catch (e) {
  ...
  public serialize(): SerializedReminderTrigger {
    switch (this.props.type) {
      ...
      default:
        throw new Error(
          "Unsupported ReminderTrigger type during serialization"
        );
    }
  }
}
