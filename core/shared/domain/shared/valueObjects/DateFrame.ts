...
export class DateFrame extends ValueObject<IDateFrame> {
  ...
  validate(props: IDateFrame): void {
    if (
      Guard.isEmpty(props.start).succeeded ||
      Guard.isEmpty(props.end).succeeded
    )
      throw new ArgumentNotProvidedException(
        "The start and end date must be provide."
      );
    if (!props.start.isBefore(props.end))
      throw new ArgumentInvalidException(
        "The start date must be less than end date or equal to it."
      );
  }
