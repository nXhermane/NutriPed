...
import {
  DiagnosticModificationDto,
  NutritionalAssessmentResultDto,
  NutritionalDiagnosticDto,
  PatientDiagnosticDataDto,
} from "../dtos";

export class NutritionalDiagnosticMapper
  implements ApplicationMapper<NutritionalDiagnostic, NutritionalDiagnosticDto>
{
  constructor(
    private readonly patientDataMapper: ApplicationMapper<
      PatientDiagnosticData,
      PatientDiagnosticDataDto
    >,
    private readonly nutritionalAssessmentMapper: ApplicationMapper<
      NutritionalAssessmentResult,
      NutritionalAssessmentResultDto
    >
  ) {}
  toResponse(entity: NutritionalDiagnostic): NutritionalDiagnosticDto {
    return {
      ...
      modificationHistories: entity
        .getModificationHistories()
        .map(this.mapDiagnosticModification),
      atInit: entity.getDiagnosticResult() ? false : true,
      result: entity.getDiagnosticResult()
        ? this.nutritionalAssessmentMapper.toResponse(
            entity.getDiagnosticResult() as NutritionalAssessmentResult
          )
        : undefined,
  ...
  private mapDiagnosticModification(
    diagnosticModification: DiagnosticModification
  ): DiagnosticModificationDto {
    const { date, nextResult, prevResult, reason } =
      diagnosticModification.unpack();
    return {
      date: date.unpack(),
