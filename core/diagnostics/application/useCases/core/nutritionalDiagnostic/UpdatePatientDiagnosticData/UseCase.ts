...
export class UpdatePatientDiagnosticDataUseCase
  implements
    UseCase<
    ...
    >
{
  ...
  async execute(
    request: UpdatePatientDiagnosticDataRequest
    ...
    try {
      ...
      const updatedRes = this.updatePatientDiagnosticData(
        nutritionalDiagnostic,
        request.patientDiagnosticData
      );
      if (updatedRes.isFailure) return left(updatedRes);
      const validationResult = await this.validatePatientDiagnosticData(
        nutritionalDiagnostic
      );
      ...
      return right(
        Result.ok(
          this.mapper.toResponse(nutritionalDiagnostic.getPatientData())
        )
      );
    } catch (e: unknown) {
  ...
  private mergeAnthropometricData(
    ...
    return AnthropometricData.create({
      anthropometricMeasures: mergedMeasures,
    });
  }
    ...
    for (const sign of newData.clinicalSigns) {
      ...
      if (existingIndex !== -1) {
        ...
        if (signResult.isFailure)
          return Result.fail(
            formatError(signResult, UpdatePatientDiagnosticDataUseCase.name)
          );
  ...
  private mergeBiologicalData(
    ...
    for (const result of newData) {
      ...
      if (newResultOrError.isFailure)
        return Result.fail(
          formatError(newResultOrError, UpdatePatientDiagnosticDataUseCase.name)
        );
  ...
  private async validatePatientDiagnosticData(
    nutritionalDiagnostic: NutritionalDiagnostic
  ): Promise<Result<boolean>> {
    try {
      const { atInit, patientData } = nutritionalDiagnostic.getProps();
      if (!atInit) return Result.ok(true);
      const patientDataValidationResult =
        await this.patientValidationService.validate(patientData);
      if (patientDataValidationResult.isFailure)
        return Result.fail(
          formatError(
            patientDataValidationResult,
            UpdatePatientDiagnosticDataUseCase.name
          )
        );
