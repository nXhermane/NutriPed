...
export class ValidateMeasurementsUseCase
  implements UseCase<ValidateMeasurementsRequest, ValidateMeasurementsResponse>
{
  constructor(
  ...
  async execute(
    request: ValidateMeasurementsRequest
    ...
    try {
      ...
      const { id, gender, birthday, clinicalSigns } = nutritionalDiagnostic
        .getPatientData()
        .getProps();

      const measuresResult = await this.buildMeasurements(
        request,
        clinicalSigns
      );
  ...
  private async buildMeasurements(
    request: ValidateMeasurementsRequest,
    clinicalSigns: ClinicalData
  ): Promise<Result<MeasurementsData>> {
    ...
    if (combinedRes.isFailure)
      return Result.fail(
        formatError(combinedRes, ValidateMeasurementsUseCase.name)
      );
