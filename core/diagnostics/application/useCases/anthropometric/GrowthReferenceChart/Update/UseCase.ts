...
export class UpdateGrowthReferenceChartUseCase
  implements
    UseCase<
    ...
    >
{
  ...
  async execute(
    request: UpdateGrowthReferenceChartRequest
  ): Promise<UpdateGrowthReferenceChartResponse> {
    try {
  ...
  private updateGrowthRef(
    growthRef: GrowthReferenceChart,
    data: UpdateGrowthReferenceChartRequest["data"]
  ): Result<unknown> {
    try {
      if (data.sex) {
