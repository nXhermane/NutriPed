...
export class CreateIndicatorUseCase
  implements UseCase<CreateIndicatorRequest, CreateIndicatorResponse>
{
  constructor(
  ...
  async execute(
    ...
    try {
      ...
      const exist = await this.indicatorRepo.exist(
        indicator.val.getProps().code
      );
      if (exist)
        return left(
          Result.fail(
            `The indicator with this code [${indicator.val.getCode()}] already exist.`
          )
        );
