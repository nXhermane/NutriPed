...
export class UpdateGrowthReferenceTableUseCase
  implements
    UseCase<
    ...
    >
{
  ...
  async execute(
    request: UpdateGrowthReferenceTableRequest
  ): Promise<UpdateGrowthReferenceTableResponse> {
    try {
  ...
  private updateGrowthRef(
    growthRef: GrowthReferenceTable,
    data: UpdateGrowthReferenceTableRequest["data"]
  ): Result<unknown> {
    try {
      if (data.name) {
