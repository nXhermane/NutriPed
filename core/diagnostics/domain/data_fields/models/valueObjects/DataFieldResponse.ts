import { ArgumentNotProvidedException, formatError, Guard, handleError, Result, SystemCode, ValueObject } from "@/core/shared";

export type DataFieldResponseValue = boolean | number | string | { unit: string, value: number }
...
export class DataFieldResponse extends ValueObject<IDataFieldResponse> {
    ...
    protected validate(props: Readonly<IDataFieldResponse>): void {
        if (Guard.isEmpty(props.value).succeeded) {
            throw new ArgumentNotProvidedException("The value of data field response can't be empty.Please provide it.")
        }
    }
