...
export class GrowthReferenceSelectionService
  implements IReferenceSelectionService
{
  constructor(
    private growthReferenceChartRepo: GrowthReferenceChartRepository,
    private growthReferenceTableRepo: GrowthReferenceTableRepository
  ) {}
  async selectTableForIndicator(
    ...
    try {
      ...
      const selectedTable = availableTables.find(table =>
        this.validateCondition(table, data, standard)
      );
      if (!selectedTable) {
      ...
      const growthReferenceTable =
        await this.growthReferenceTableRepo.getByCode(
          selectedTable.unpack().tableCode
        );
  ...
  private validateCondition(
    availableRef: AvailableChart | AvailableTable,
    data: AnthropometricVariableObject,
    standard: GrowthStandard
  ): boolean {
    ...
    return (
      evaluateCondition(condition.unpack().value, { ...data, standard }) ===
  ...
  async selectChartForIndicator(
    ...
    try {
      ...
      const selectedChart = availableCharts.find(chart =>
        this.validateCondition(chart, data, standard)
      );
      ...
      const growthReferenceChart =
        await this.growthReferenceChartRepo.getByCode(
          selectedChart.unpack().chartCode
        );
