  ...
  protected validate(props: Readonly<IIndicatorInterpreter>): void {
    if (Guard.isEmpty(props.name).succeeded)
      throw new EmptyStringError(
        "The name of ChartInterpretation can't be empty."
      );
  }

  static create(
    createProps: CreateIndicatorInterpreter
  ): Result<IndicatorInterpreter> {
    try {
      ...
      if (combinedRes.isFailure)
        return Result.fail(formatError(combinedRes, IndicatorInterpreter.name));
      return Result.ok(
