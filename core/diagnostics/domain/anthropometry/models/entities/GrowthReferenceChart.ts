  ...
  public validate(): void {
    ...
    if (Guard.isEmpty(this.props.name).succeeded)
      throw new EmptyStringError(
        "The name of GrowthReferenceChart can't be empty."
      );
    this._isValid = true;
  }

  static create(
    createProps: CreateGrowthReferenceChartProps,
    id: AggregateID
  ): Result<GrowthReferenceChart> {
    try {
      const codeRes = SystemCode.create(createProps.code);
      const chartDataRes = createProps.data.map(chartData =>
        ChartData.create(chartData)
      );
      const combinedRes = Result.combine([codeRes, ...chartDataRes]);
      if (combinedRes.isFailure)
        return Result.fail(formatError(combinedRes, GrowthReferenceChart.name));
