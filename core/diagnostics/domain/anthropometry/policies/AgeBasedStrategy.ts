...
export class AgeBasedStrategy extends AbstractZScoreComputingStrategy {
  ...
  computeZScore<T extends GrowthReferenceChart | GrowthReferenceTable>(
    ...
    if (age_in_day <= MAX_AGE_TO_USE_AGE_IN_DAY) {
      const standard = this.findGrowthStandardWhenAgeIsInDay(
        age_in_day,
        growthReferenceChart as GrowthReferenceChart
      );
      if (!standard) return NaN;
      return Math.round(
        this.anthroComputingHelper.computeZScoreAdjusted(
          y,
          standard.l,
          standard.median,
          standard.s
    ...
    if (age_in_day <= MAX_AGE_IN_PEDIATRIC * DAY_IN_YEARS) {
      ...
      const standard = this.findGrowthStandardWhenAgeIsInMonth(
        age_in_month,
        growthReferenceChart as GrowthReferenceChart
      );
      if (!standard) return NaN;
  ...
  private findGrowthStandardWhenAgeIsInDay(
    age_in_day: number,
    growthReferenceChart: GrowthReferenceChart
  ): IChartData | undefined {
    const standard = growthReferenceChart
      .getChartData()
      .find(chartData => chartData.value === age_in_day);
    return standard;
  }
  private findGrowthStandardWhenAgeIsInMonth(
    age_in_month: number,
    growthReferenceChart: GrowthReferenceChart
  ): IChartData | undefined {
    ...
    if (standardUpp && standardLow) {
      return {
        ...
        l:
          diffAge > 0
            ? standardLow.l + diffAge * (standardUpp.l - standardLow.l)
            : standardLow.l,
        s:
          diffAge > 0
            ? standardLow.s + diffAge * (standardUpp.s - standardLow.s)
            : standardLow.s,
