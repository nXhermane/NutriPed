...
export class NutritionalDiagnosticFactory
  ...
  async create(
    ...
    try {
      const patientDiagnosticDataResult =
        await this.createAndValidatePatientData(props.patientDiagnosticData);
      const domainDate = DomainDate.create();
      const combinedRes = Result.combine([
        patientDiagnosticDataResult,
        domainDate,
      ]);
      if (combinedRes.isFailure)
        return handleDiagnosticCoreError(
          CORE_SERVICE_ERRORS.NUTRITIONAL_DIAGNOSTIC_FACTORY.CREATION_FAILED
            .path,
  ...
  private async createAndValidatePatientData(
    ...
    try {
      ...
      const patientValidationResult =
        await this.patientDataValidationService.validate(
          patientDiagnosticDataResult.val
        );
      if (patientValidationResult.isFailure)
        return Result.fail(
          formatError(
            patientValidationResult,
            NutritionalDiagnosticFactory.name
          )
        );
      return patientDiagnosticDataResult;
    } catch (e: unknown) {
