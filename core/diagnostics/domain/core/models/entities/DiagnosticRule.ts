...
export class DiagnosticRule extends Entity<IDiagnosticRule> {
  ...
  public validate(): void {
    ...
    if (Guard.isEmpty(this.props.name).succeeded)
      throw new EmptyStringError("The name of Diagnostic Rule can't be empty.");
    if (this.props.conditions.length < DiagnosticRule.MIN_CONDITION)
      throw new ArgumentOutOfRangeException(
        "The diagnostic Rule must contains DiagnosticCondition."
      );
    this._isValid = true;
  }

  static create(
    createProps: CreateDiagnosticRule,
    id: AggregateID
  ): Result<DiagnosticRule> {
    try {
      const codeRes = SystemCode.create(createProps.code);
      const diagnosticConditionsRes = createProps.conditions.map(
        DiagnosticCondition.create
      );
      const combinedRes = Result.combine([codeRes, ...diagnosticConditionsRes]);
      if (combinedRes.isFailure)
        return Result.fail(formatError(combinedRes, DiagnosticRule.name));
      return Result.ok(
        new DiagnosticRule({
          id,
          props: {
