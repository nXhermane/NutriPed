...
export class OrientationRefMapper
  implements ApplicationMapper<OrientationReference, OrientationRefDto>
{
  toResponse(entity: OrientationReference): OrientationRefDto {
    return {
      ...
      admissionTypes: entity.getAdmissionTypes().map(valObj => ({
        code: valObj.code.unpack(),
        condition: valObj.condition.unpack(),
        name: valObj.name,
      })),
      code: entity.getCode(),
