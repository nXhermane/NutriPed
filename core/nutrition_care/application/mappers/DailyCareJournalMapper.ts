...
export class DailyCareJournalMapper
  implements ApplicationMapper<DailyCareJournal, DailyCareJournalDto>
{
  toResponse(entity: DailyCareJournal): DailyCareJournalDto {
    return {
      ...
      appetiteTestResults: entity
        ...
        .map(valObj => ({ code: valObj.code.unpack(), result: valObj.result })),
      observations: entity.getObservations().map(valObj => ({
        code: valObj.code.unpack(),
        isPresent: valObj.isPresent,
        type: valObj.type,
      })),
      monitoringValues: entity.getMonitoringValues().map(valObj => ({
        ...valObj,
        date: valObj.date.unpack(),
        code: valObj.code.unpack(),
