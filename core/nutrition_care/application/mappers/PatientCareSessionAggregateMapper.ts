...
export class PatientCareSessionAggregateMapper
  implements
    ApplicationMapper<PatientCareSession, PatientCareSessionAggregateDto>
{
  constructor(
    private readonly carePhaseMapper: ApplicationMapper<
      ...
      CarePhaseDto
    >,
    private readonly dailyRecordMapper: ApplicationMapper<
      DailyCareRecord,
      DailyCareRecordDto
    >,
    private readonly messageMapper: ApplicationMapper<Message, MessageDto>
  ) {}
  toResponse(entity: PatientCareSession): PatientCareSessionAggregateDto {
    ...
    return {
      ...
      currentPhase: currentPhase
        ? this.carePhaseMapper.toResponse(currentPhase)
        : null,
      currentDailyRecord: currentDailyRecord
        ? this.dailyRecordMapper.toResponse(currentDailyRecord)
        : null,
      phaseHistory: phaseHistory.map(phase =>
        this.carePhaseMapper.toResponse(phase)
      ),
      dailyRecords: dailyRecords.map(record =>
        this.dailyRecordMapper.toResponse(record)
      ),
