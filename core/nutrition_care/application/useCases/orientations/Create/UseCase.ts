...
export class CreateOrientationRefUseCase
  implements UseCase<CreateOrientationRefRequest, CreateOrientationRefResponse>
{
  constructor(
  ...
  async execute(
    request: CreateOrientationRefRequest
  ): Promise<CreateOrientationRefResponse> {
    try {
      const newId = this.idGenerator.generate().toValue();
      const orientationRefRes = OrientationReference.create(
        request.data,
        newId
      );
      ...
      if (exist)
        return left(
          Result.fail(
            `The orientationRef with this code [${orientationRef.getCode()}] already exist.`
          )
        );
