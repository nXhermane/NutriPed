...
export class GetNutritionalProductUseCase
  implements UseCase<GetNutritionalProductRequest, GetNutritionalProductResponse>
{
  constructor(
    private readonly repo: NextNutritionCare.NutritionalProductRepository,
    private readonly mapper: ApplicationMapper<NextNutritionCare.NutritionalProduct,NutritionalProductDto>
  ) {}
  async execute(request: GetNutritionalProductRequest): Promise<GetNutritionalProductResponse> {
    try {
      ...
      return right(
        Result.ok(nutritionalProducts.map(product => this.mapper.toResponse(product)))
      );
    } catch (e: unknown) {
