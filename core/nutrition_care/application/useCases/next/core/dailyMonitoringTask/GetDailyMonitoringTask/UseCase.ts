...
export class GetDailyMonitoringTaskUseCase
  ...
  constructor(
    ...
    private readonly dailyMonitoringTaskMapper: ApplicationMapper<DailyMonitoringTask,DailyMonitoringTaskDto>
  ) { }
  ...
  async execute(
    ...
    try {
      const entity = await this.dailyMonitoringTaskRepository.getById(request.dailyMonitoringTaskId);
