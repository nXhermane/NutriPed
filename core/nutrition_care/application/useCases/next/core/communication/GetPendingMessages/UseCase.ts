...
export class GetPendingMessagesUseCase
  implements UseCase<GetPendingMessagesRequest, GetPendingMessagesResponse>
{
  constructor(
    private readonly messageMapper: ApplicationMapper<
      NextCore.Message,
      MessageDto
    >,
  ...
  async execute(
    ...
    try {
      const messageRes = await this.orchestratorPort.getPendingMessages(
        request.sessionId
      );
      if (messageRes.isFailure) {
        return left(messageRes);
      }
      const messages = messageRes.val.map(message =>
        this.messageMapper.toResponse(message)
      );
      return right(
        Result.ok({
