...
export class GetComplicationUseCase
  implements UseCase<GetComplicationRequest, GetComplicationResponse>
{
  constructor(
    private readonly complicationRepository: ComplicationRepository,
    private readonly complicationMapper: ApplicationMapper<
      Complication,
      ComplicationDto
    >
  ) {}
  ...
  async execute(
    request: GetComplicationRequest
  ): Promise<GetComplicationResponse> {
    try {
      const complications: Complication[] = [];

      if (request.complicationId) {
        const complication = await this.complicationRepository.getById(
          request.complicationId
        );
      ...
      return right(
        Result.ok(complications.map(this.complicationMapper.toResponse))
      );
    } catch (e: unknown) {
