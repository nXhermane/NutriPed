    ...
    UseCase<CreatePatientCareSessionRequest, CreatePatientCareSessionResponse>
{
  ...
  async execute(
    ...
    try {
      ...
      if (!patientExist) {
        return left(
          Result.fail("PatientCareSession must be created for valid patient.")
        );
      }
  ...
  private createNoneAppetiteTestResult() {
    return new AppetiteTestResult({
      code: new SystemCode({ _value: APPETITE_TEST_CODES.CODE }),
      result: APPETITE_TEST_RESULT_CODES.NONE,
    });
  }
