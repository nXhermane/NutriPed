...
import { CarePhaseReferenceRepository, CarePhaseReference } from "@/core/nutrition_care/domain/modules/carePhase";

export class GetCarePhaseReferenceUseCase
  implements UseCase<GetCarePhaseReferenceRequest, GetCarePhaseReferenceResponse>
{
  constructor(
  ...
  async execute(request: GetCarePhaseReferenceRequest): Promise<GetCarePhaseReferenceResponse> {
    try {
      const carePhases =  []
      ...
      return right(
        Result.ok(carePhases.map(carePhase => this.mapper.toResponse(carePhase)))
      );
    } catch (e: unknown) {
