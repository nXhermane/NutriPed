...
export class PatientCareSessionFactory
  implements Factory<CreatePatientCareSessionProps, PatientCareSession>
{
  constructor(
    private idGenerator: GenerateUniqueId,
    private readonly patientDailyJournalGenerator: IPatientDailyJournalGenerator
  ) {}
  async create(
    props: CreatePatientCareSessionProps
    ...
    try {
      ...
      const defaultOrientation = {
        ...
        code: new SystemCode({
          _value: ORIENTATION_REF_CODES.ORIENTED_TO_HOME,
        }),
      };
      ...
      if (patientCurrentStateRes.isFailure)
        return Result.fail(
      ...
      if (result.isFailure)
        return Result.fail(formatError(result, PatientCareSessionFactory.name));
