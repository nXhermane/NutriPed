...
export class PatientDailyJournalGenerator
  implements IPatientDailyJournalGenerator
{
  ...
  createDailyJournalIfNeeded(
    ...
    try {
      if (!patientCareSession.haveCurrentDailyJournal()) {
        ...
        const dailyJournalRes = DailyCareJournal.create(
          patientCareSession.getStartDate(),
          newId
        );
        if (dailyJournalRes.isFailure) {
          return Result.fail(
            formatError(dailyJournalRes, PatientDailyJournalGenerator.name)
          );
        }
