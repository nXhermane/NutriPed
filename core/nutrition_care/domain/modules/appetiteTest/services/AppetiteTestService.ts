  ...
  async test(data: AppetiteTestData): Promise<Result<AppetiteTestResult>> {
    try {
      ...
      const appetiteTestTableRange = appetiteTestRef.val
        ...
        .find(
          range =>
            range.weightRange[0] <= patientWeight &&
            range.weightRange[1] >= patientWeight
        );
      if (!appetiteTestTableRange) {
        return handleAppetiteTestError(
          APPETITE_TEST_ERRORS.SERVICE
            .APPETITE_TEST_RANGE_NOT_FOUND_FOR_THE_GIVEN_WEIGHT.path,
      ...
      if (data.productTypeIsSachet()) {
        ...
        const sachetTakenAmount = Number(
          evaluateFormula((takenAmount as TakenAmountInSachet).takenFraction)
        );
        return AppetiteTestResult.create({
          code: APPETITE_TEST_CODES.CODE,
          result:
            min <= sachetTakenAmount
