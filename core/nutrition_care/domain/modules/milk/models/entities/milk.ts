...
export class Milk extends Entity<IMilk> {
  ...
  public validate(): void {
    ...
    if (Guard.isEmpty(this.props.recommendedMilkPerDay).succeeded) {
      throw new ArgumentNotProvidedException(
        "The milk must have the recommended milk taken per day."
      );
    }
    this._isValid = true;
  }
  ...
  static create(createProps: CreateMilkProps, id: AggregateID): Result<Milk> {
    try {
      ...
      const rangesRes = createProps.recommendationPerRanges.map(
        RecommendedMilkPerWeightRange.create
      );
      ...
      if (combinedRes.isFailure)
        return Result.fail(formatError(combinedRes, Milk.name));
      const milk = new Milk({
        id,
        props: {
