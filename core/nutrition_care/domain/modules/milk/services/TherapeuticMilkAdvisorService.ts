  ...
  implements ITherapeuticMilkAdvisorService
{
  suggest(
    ...
    try {
      ...
      if (milkSuggestedResult.isFailure)
        return Result.fail(
          formatError(milkSuggestedResult, TherapeuticMilkAdvisorService.name)
        );
  ...
  private getMilkByCondition(
    ...
    try {
      for (const milk of milks) {
        ...
        const conditionRes = evaluateCondition(
          condition.value,
  ...
  private getMilkSuggestionResult(
    input: MilkSuggestionInput,
    milk: Milk
  ): Result<MilkSuggestionResult> {
    try {
      ...
      const calculatedVolume = evaluateFormula(
        volumeFormula.value,
        volumeContext as never
      ) as number;
      ...
      const feedingFrequencies = Object.keys(
  ...
  private getRange(
    input: MilkSuggestionInput,
    ranges: IRecommendedMilkPerWeightRange[]
  ): IRecommendedMilkPerWeightRange | undefined {
