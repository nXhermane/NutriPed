  ...
  protected validate(props: Readonly<IMedicineDosageResult>): void {
    ...
    if (props.dailyDosageFrequency < 1) {
      throw new ArgumentOutOfRangeException(
        "The medicine taken frequency must be greater than zero."
      );
    }
  }

  static create(
    createProps: CreateMedicineDosageResult
  ): Result<MedicineDosageResult> {
    try {
      const codeRes = SystemCode.create(createProps.code);
      const weightRangeDosageRes = DosageRange.create(
        createProps.weightRangeDosage
      );
      const combinedRes = Result.combine([codeRes, weightRangeDosageRes]);
      if (combinedRes.isFailure)
        return Result.fail(formatError(combinedRes, MedicineDosageResult.name));
