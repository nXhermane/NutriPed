...
export class OrientationService implements IOrientationService {
  orient(
    ...
    try {
      ...
      if (orientationResults.length === 0)
        return Result.fail(
          "The orientation failed. Please check if all needed measure is provided."
        );
  ...
  private choseOrientationResult(
    orientationResults: OrientationResult[]
    ...
    for (const orientationResult of orientationResults) {
      const currentPriority =
        ORIENTATION_PRIORITY[
          orientationResult.code.unpack() as keyof typeof ORIENTATION_PRIORITY
        ] || 0;
