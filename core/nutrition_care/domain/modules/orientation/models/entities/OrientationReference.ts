...
export class OrientationReference extends Entity<IOrientationReference> {
  ...
  public validate(): void {
    ...
    if (Guard.isEmpty(this.props.admissionCriteria).succeeded) {
      throw new ArgumentNotProvidedException(
        "The orientation reference must have greater than zero admission criteria."
      );
    }
    this._isValid = true;
  }
  ...
  static create(
    createProps: CreateOrientationReferenceProps,
    id: AggregateID
  ): Result<OrientationReference> {
    try {
      const codeRes = SystemCode.create(createProps.code);
      const admissionCriteriaRes = createProps.admissionCriteria.map(
        Condition.create
      );
      const admissionTypesRes = createProps.admissionTypes.map(
        AdmissionType.create
      );
      const combinedRes = Result.combine([
        codeRes,
        ...admissionCriteriaRes,
        ...admissionTypesRes,
      ]);
