...
import { ITriggerExecutor, TriggerExecutionResult } from "./interfaces";

export class TriggerExecutor implements ITriggerExecutor {
  constructor(
        ...
        if (recommendedTreatment) {
          ...
          for (const trigger of onStartTriggers) {
            ...
            if (executionResult.isFailure) {
              errors.push(
                `Failed to execute onStart trigger for treatment ${treatment.id}: ${executionResult.err}`
              );
            } else {
              executedCount++;
            }
      ...
      for (const treatment of treatments) {
        ...
        if (recommendedTreatment) {
          ...
          for (const trigger of onEndTriggers) {
            ...
            if (executionResult.isFailure) {
              errors.push(
                `Failed to execute onEnd trigger for treatment ${treatment.id}: ${executionResult.err}`
              );
            } else {
              executedCount++;
            }
  ...
  private async executeTrigger(
    ...
    try {
      ...
      const recommendedTreatmentRes =
        await this.recommendedTreatmentService.getByRecommendationCode(
          trigger.targetTreatment as ValueOf<typeof TREATMENT_PLAN_IDS>
        );

      if (recommendedTreatmentRes.isFailure) {
        return Result.fail(
          `Failed to get recommended treatment for code ${trigger.targetTreatment}: ${recommendedTreatmentRes.err}`
        );
      }
      ...
      const targetTreatment = existingTreatments.find(
        ot =>
          ot.getRecommendation().recommendationCode.unpack() ===
          trigger.targetTreatment
      );
      ...
      switch (trigger.action) {
        case TREATMENT_TRIGGER_ACTION.START:
          if (targetTreatment) {
            ...
            if (
              targetTreatment.getStatus() === OnGoingTreatmentStatus.STOPPED
            ) {
              targetTreatment.activeTreatment();
              carePhase.changeOnGoingTreatment(targetTreatment);
              console.log(
                `Activated existing treatment ${trigger.targetTreatment} via START trigger`
              );

              // Exécuter récursivement les onStart triggers de ce traitement
              await this.executeTreatmentTriggers(
                targetTreatment,
              ...
              console.log(
                `Treatment ${trigger.targetTreatment} is already active`
              );
            }
          } else {
            // Si le traitement n'existe pas, le créer et l'ajouter
            const newTreatmentRes =
              this.createOnGoingTreatmentFromRecommended(recommendedTreatment);
            if (newTreatmentRes.isFailure) {
              return Result.fail(
                `Failed to create new treatment ${trigger.targetTreatment}: ${newTreatmentRes.err}`
              );
            }
            carePhase.addOnGoingTreatment(newTreatmentRes.val);
            console.log(
              `Created and added new treatment ${trigger.targetTreatment} via START trigger`
            );
        ...
        case TREATMENT_TRIGGER_ACTION.STOP:
          if (targetTreatment) {
            ...
            if (targetTreatment.getStatus() === OnGoingTreatmentStatus.ACTIVE) {
              ...
              carePhase.changeOnGoingTreatment(targetTreatment);
              console.log(
                `Stopped treatment ${trigger.targetTreatment} via STOP trigger`
              );
            } else {
              console.log(
                `Treatment ${trigger.targetTreatment} is already stopped`
              );
            }
          } else {
            console.log(
              `Treatment ${trigger.targetTreatment} not found in CarePhase, nothing to stop`
            );
          }
          break;
  ...
  private async executeTreatmentTriggers(
    ...
    try {
      ...
      for (const trigger of triggers) {
        ...
        if (executionResult.isFailure) {
          console.warn(
            `Failed to execute ${triggerType} trigger for treatment ${treatment.id}: ${executionResult.err}`
          );
        }
      }
    } catch (e: unknown) {
      console.warn(
        `Error executing ${triggerType} triggers for treatment ${treatment.id}:`,
        e
      );
    }
  }
  ...
  async executeAllTriggers(
    ...
    try {
      const onStartResult = await this.executeOnStartTriggers(
        newTreatments,
        recommendedTreatments,
        carePhase
      );
      const onEndResult = await this.executeOnEndTriggers(
        stoppedTreatments,
        recommendedTreatments,
        carePhase
      );
      ...
      return Result.ok({
        ...
        errors: [...onStartResult.val.errors, ...onEndResult.val.errors],
      });
    } catch (e: unknown) {
      return handleError(e);
    }
