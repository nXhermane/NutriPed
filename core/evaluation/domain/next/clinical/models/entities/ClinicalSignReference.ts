...
import { AggregateID, ArgumentNotProvidedException, Condition, EmptyStringError, Entity, EntityPropsBaseType, formatError, Guard, handleError, ICondition, Result, SystemCode } from "@/core/shared";
...
export class ClinicalSignReference extends Entity<IClinicalSignReference> {
    ...
    changeDataFields(dataFields: { code: DATA_FIELD_CODE_TYPE, required: boolean }[]) {
    ...
    public validate(): void {
        ...
        if (Guard.isEmpty(this.props.name).succeeded) {
            throw new EmptyStringError("The name of clinical sign reference can't be empty.")
        }
        if (Guard.isEmpty(this.props.description).succeeded) {
            throw new EmptyStringError("The description of clinical sign reference can't be empty.")
        }
        if (Guard.isEmpty(this.props.neededDataFields).succeeded) {
            throw new ArgumentNotProvidedException("The needed Data fields can't be emtpy.")
        }
        this._isValid = true
    }
    static create(createProps: CreateClinicalSignReference, id: AggregateID): Result<ClinicalSignReference> {
        try {
            ...
            if (combinedRes.isFailure) {
                return Result.fail(formatError(combinedRes, ClinicalSignReference.name))
            }
            return Result.ok(new ClinicalSignReference({
                id, props: {
