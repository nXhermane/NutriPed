  ...
  implements IClinicalVariableGeneratorService
{
  ...
  async generate(
    ...
    try {
      const clinicalRefsCodes = (await this.clinicalRefRepo.getAllCode()).map(
        code => code.unpack()
      );
      const clinicalVariableObject: ClinicalVariableObject = {};
      const clinicalEvaluationResultRecord = clinicalEvaluationResults.reduce(
        (acc: any, current) => {
          acc[current.code.unpack()] = current.isPresent
      ...
      for (const clinicalRefCode of clinicalRefsCodes) {
        clinicalVariableObject[clinicalRefCode] =
          clinicalEvaluationResultRecord[clinicalRefCode] == undefined
            ? ConditionResult.False
            : clinicalEvaluationResultRecord[clinicalRefCode];
      }
