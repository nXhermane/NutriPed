...
export class ClinicalValidationService implements IClinicalValidationService {
  constructor(
    private readonly dataFieldValidation: DataFieldValidationService,
  ...
  async validate(
    ...
    try {
      const clinicalValidationResults = await Promise.all(
        clinicalData.map(data => this.validateClinicalData(data))
      );
      const combinedRes = Result.combine(clinicalValidationResults as any);
      if (combinedRes.isFailure) {
        return Result.fail(
          formatError(combinedRes, ClinicalValidationService.name)
        );
      }
      return Result.ok({
        isValid: true,
    ...
    try {
      ...
      for (const dataField of neededDataFields) {
        if (dataField.required && !(data[dataField.code] == undefined)) {
          return handleClinicalError<void>(
            CLINICAL_ERRORS.VALIDATION.MISSING_DATA.path,
            `[MISSING_DATA_CODE]: ${dataField.code}`
          );
        }
      }
      const dataFieldsResponseResult = Object.entries(data).map(
        ([code, value]) =>
      ...
      const dataFieldValidationResult = await this.dataFieldValidation.validate(
        dataFieldsResponseResult.map(res => res.val)
      );
