...
export class AppetiteTestData extends ValueObject<IAppetiteTestData> {
  ...
  protected validate(props: Readonly<IAppetiteTestData>): void {
    ...
    if (props.givenProductType === APPETITE_TEST_PRODUCT_TYPE.IN_POT) {
      if (
        Guard.isEmpty((props.takenAmount as TakenAmountOfPot).takenQuantity)
          .succeeded
      )
        throw new ArgumentInvalidException(
        ...
        Guard.isNegative((props.takenAmount as TakenAmountOfPot).takenQuantity)
          .succeeded
      )
    ...
    if (
      Guard.isNegative(props.patientWeight).succeeded ||
      props.patientWeight === 0
    ) {
      throw new NegativeValueError(
        "The patient Weight can't be negative or equal to zero."
      );
    }
  }
