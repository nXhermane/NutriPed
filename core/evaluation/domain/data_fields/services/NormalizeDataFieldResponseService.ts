  ...
  implements INormalizeDataFieldResponseService
{
  ...
  async normalizeAndFillDefaults(
    ...
    try {
      ...
      const fields = [
        ...dataFieldResponse,
        ...unincludedDataFieldResponseRes.val,
      ];
  ...
  private async getOnlyUnincludedFields(
    codeMap: Set<string>
  ): Promise<Result<DataFieldResponse[]>> {
    try {
      const fields = await this.dataFieldRepo.getAll();
      const filteredFields = fields.filter(
        field => !codeMap.has(field.getCode())
      );
