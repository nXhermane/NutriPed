...
export class DataFieldValidationService implements IDataFieldValidatationService {
    validate(data: DataFieldResponse[], dataFieldReferences: DataFieldReference[]): Result<void> {
        ...
        if (combinedResult.isFailure) {
            return Result.fail(formatError(combinedResult, DataFieldValidationService.name))
        }
        return Result.ok(void 0)
    }

    private validateDataFieldResponse(data: DataFieldResponse, dataFieldRefs: DataFieldReference[]): Result<boolean> {
        try {
            const findedRefs = dataFieldRefs.find(field => field.getCode() == data.getCode())
            if (!findedRefs) {
                return handleDataFieldError(DATA_FIELD_ERROS.VALIDATION.NOT_FOUND.path, `DataFieldCode: ${data.getCode()}`)
            }
            ...
            switch (findedRefs.getType()) {
                ...
                case FieldDataType.RANGE:
                    {
                        ...
                        const inRange = Guard.inRange(data.getValue() as number, findedRefs.getRange()?.[0] as number, findedRefs.getRange()?.[1] as number, 'dataField').succeeded
                ...
                case FieldDataType.ENUM: {
                    ...
                    validationResult = !!enumValues.find(enumItem => enumItem.value == value)

                } break;
                case FieldDataType.QUANTITY: {
                    const dataValue: any = data.getValue()
                    const isQuantityValue = typeof dataValue == 'object' && 'data' in dataValue && 'unit' in dataValue
                    const haveValidUnit = findedRefs.getUnits()!.available.find(unit => unit == dataValue.unit)
            ...
            if (!validationResult) {
                return handleDataFieldError(DATA_FIELD_ERROS.VALIDATION.INVALID_DATA_FIELD.path, `[DataFieldCode]: ${data.getCode()}`)
            }
