...
import { AggregateID, ArgumentInvalidException, ArgumentNotProvidedException, ArgumentOutOfRangeException, EmptyStringError, Entity, EntityPropsBaseType, formatError, Guard, handleError, Result, SystemCode, UnitCode } from "@/core/shared";
...
export class DataFieldReference extends Entity<IDataFieldReference> {
    ...
    changeData(data: {
        ...
        range?: [number, number]
        units?: {
            default: UnitCode, available: UnitCode[]
        }
    }) {
        ...
        switch (this.props.type) {
            case FieldDataType.ENUM:
                {
                    if (Guard.isEmpty(this.props.enum).succeeded) {
                        throw new ArgumentNotProvidedException("The enumValue can't be emtpy if the type of DataField is an Enum.")
                    }
                    const enumValueContaintDefaultValue = this.props.enum?.some(enumItem => enumItem === this.props.defaultValue)
                    if (!enumValueContaintDefaultValue) {
                        throw new ArgumentNotProvidedException("La valeur par defaut d'un DataField de type enum doit etre contnue dans les options possible pour l'enum.")
                    }
                }
                break;
            ...
            case FieldDataType.RANGE: {
                if (Guard.isEmpty(this.props.range).succeeded) {
                    throw new ArgumentNotProvidedException("The range of Ranged DataField can't be empty.")
                }
                if (!Guard.inRange(this.props.defaultValue, this.props.range?.[0] as number, this.props.range?.[1] as number, 'range').succeeded) {
                    throw new ArgumentOutOfRangeException("The default value of Ranged DataField must be in interval of provided range.")
                }

            } break;
            case FieldDataType.QUANTITY: {
                if (Guard.isEmpty(this.props.units).succeeded) {
                    throw new ArgumentNotProvidedException("The quantity dataField must be containt on untis props.Please provided it.")
                }
                const availableUnits = this.props.units?.available
                const defaultUnit = this.props.units?.default
                const isContaintInAvailableUnits = !!availableUnits?.find(unit => unit.equals(defaultUnit))
                if (!isContaintInAvailableUnits) {
                    throw new ArgumentInvalidException("The defaults units must be containt in availables units.")
                }
                if (!Guard.isNumber(this.props.defaultValue).succeeded) {
                    throw new ArgumentInvalidException("The provided default value is not valide because the Quantity DataField can't be accepts other value different to number. ")
                }
            } break;
            case FieldDataType.BOOL: {
                if (typeof this.props.defaultValue != 'boolean') {
                    throw new ArgumentInvalidException("The boolean data Field default value can't be other than true / false")
                }
            } break;
            case FieldDataType.INT: {
                if (!Guard.isNumber(this.props.defaultValue).succeeded) {
                    throw new ArgumentInvalidException("The integer DataField can't be accepts other value than a number.")
                }
            } break;
            case FieldDataType.STR: {
                if (typeof this.props.defaultValue != 'string') {
                    throw new ArgumentInvalidException("The string DataField can't accepts other value than string.")
                }
            } break
            default: {
                throw new ArgumentOutOfRangeException("The provided Datafield Type is not supported !")
            }
        }
    ...
    static create(createProps: CreateDataFieldReference, id: AggregateID
        ...
        try {
            ...
            if (createProps.units) {
                const availableUnitRes = createProps.units.available.map(unit => UnitCode.create(unit))
                const defaultUnitsRes = UnitCode.create(createProps.units.default)
                const combinedRes = Result.combine([...availableUnitRes, defaultUnitsRes])
            ...
            return Result.ok(new DataFieldReference({
                id, props: {
