...
export class ChartData extends ValueObject<IChartData> {
  protected validate(props: Readonly<IChartData>): void {
    const isNumbers = [
      ...
      ...Object.values(props.curvePoints).map(
        val => !Guard.isNumber(val).succeeded
      ),
    ];
    if (isNumbers.some(Boolean))
      throw new ArgumentInvalidException(
        "The value|median|l|s|curvePoints must be a number. Please ensure the property is number."
      );
  }
