...
export class PatientDiagnosticData extends Entity<IPatientDiagnosticData> {
  ...
  addBiologicalTestResult(...biologicalTestResults: BiologicalTestResult[]) {
    for (const biologicalTestResult of biologicalTestResults) {
      const biologicalTestResultIndex =
        this.props.biologicalTestResults.findIndex(
          bioTestRes =>
            bioTestRes.unpack().code === biologicalTestResult.unpack().code
        );
      if (biologicalTestResultIndex != -1) {
        this.props.biologicalTestResults[biologicalTestResultIndex] =
          biologicalTestResult;
  ...
  public validate(): void {
    ...
    if (this.props.birthday.getAge() > MAX_AGE_IN_PEDIATRIC) {
      throw new ArgumentOutOfRangeException(
        `The age in pediatric must be less than ${MAX_AGE_IN_PEDIATRIC}`
      );
    }
    if (this.props.gender.isOther()) {
      throw new ArgumentInvalidException(
        `The sex in pediatric can't not be ${this.props.gender.sex}`
      );
    }
    this._isValid = true;
  }
  static create(
    createProps: CreatePatientDiagnosticData,
    id: AggregateID
  ): Result<PatientDiagnosticData> {
    try {
      const genderRes = Gender.create(createProps.sex);
      const birthDayRes = Birthday.create(createProps.birthday);
      const anthropMeasuresRes = AnthropometricData.create(
        createProps.anthropometricData
      );
      const clinicalSignsRes = ClinicalData.create(createProps.clinicalSigns);
      const biologicalTestResultsRes = createProps.biologicalTestResults.map(
        val => BiologicalTestResult.create(val)
      );
      ...
      return Result.ok(
        new PatientDiagnosticData({
          ...
          props: {
            ...
            biologicalTestResults: biologicalTestResultsRes.map(
              valRes => valRes.val
            ),
          },
        })
