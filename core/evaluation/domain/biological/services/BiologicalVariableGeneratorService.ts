...
export class BiologicalVariableGeneratorService
  implements IBiologicalVariableGeneratorService
{
  constructor(
    private readonly biologicalRefRepo: BiochemicalReferenceRepository
  ) {}
  async generate(
    interpretation: BiologicalAnalysisInterpretation[]
  ): Promise<Result<BiologicalVariableObject>> {
    try {
      const biologicalReferences = (
        await this.biologicalRefRepo.getAllCode()
      ).map(bioRef => bioRef.unpack());
      ...
      for (const biochemicalRef of biologicalReferences) {
        if (biologicalInterpretations.has(biochemicalRef)) {
          biologicalVariableObject[biochemicalRef] =
            biologicalInterpretations.get(biochemicalRef)!;
        } else {
          biologicalVariableObject[biochemicalRef] =
            BiochemicalRangeStatus.NORMAL;
        }
      }
