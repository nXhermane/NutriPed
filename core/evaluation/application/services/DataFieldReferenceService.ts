...
import { CreateDataFieldRefRequest, CreateDataFieldRefResponse, GetDataFieldRefRequest, GetDataFieldRefResponse, ValidateDataFieldResponseRequest, ValidateDataFieldResponseResponse } from "../useCases";
...
export interface DataFieldReferenceUseCases {
    ...
    validationUC: UseCase<ValidateDataFieldResponseRequest, ValidateDataFieldResponseResponse>
}
...
export class DataFieldReferenceService implements IDataFieldReferenceService {
    ...
    async create(req: CreateDataFieldRefRequest): Promise<AppServiceResponse<{ id: AggregateID; }> | Message> {
        const res = await this.ucs.createUC.execute(req);
        if (res.isRight()) return { data: res.value.val };
        else return new Message("error", JSON.stringify((res.value as any).err));
    }
    async get(req: GetDataFieldRefRequest): Promise<AppServiceResponse<DataFieldReferenceDto[]> | Message> {
        const res = await this.ucs.getUC.execute(req);
        if (res.isRight()) return { data: res.value.val };
        else return new Message("error", JSON.stringify((res.value as any).err));
    }
    async validate(req: ValidateDataFieldResponseRequest): Promise<AppServiceResponse<boolean> | Message> {
