...
import {
  ClinicalEvaluationResultDto,
  ClinicalNutritionalAnalysisDto,
} from "../../../dtos/next/clinical";
...
export class ClinicalDataAnalysisService implements IClinicalAnalysisService {
  ...
  async evaluate(
    req: NextClinicalUseCase.MakeClinicalEvaluationRequest
  ): Promise<AppServiceResponse<ClinicalEvaluationResultDto[]> | Message> {
    const res = await this.ucs.evaluateUC.execute(req);
    if (res.isRight()) return { data: res.value.val };
    else return new Message("error", JSON.stringify((res.value as any).err));
  }
  async interpret(
    req: NextClinicalUseCase.MakeClinicalInterpretationRequest
  ): Promise<AppServiceResponse<ClinicalNutritionalAnalysisDto[]> | Message> {
