...
export class GrowthReferenceChartService
  implements IGrowthReferenceChartService
{
  constructor(private readonly ucs: GrowthReferenceTChartServiceUseCases) {}
  async create(
    req: CreateGrowthReferenceChartRequest
  ...
  async get(
    req: GetGrowthReferenceChartRequest
  ): Promise<AppServiceResponse<GrowthReferenceChartDto[]> | Message> {
    const res = await this.ucs.getUC.execute(req);
    if (res.isRight()) return { data: res.value.val };
    else return new Message("error", JSON.stringify((res.value as any)?.err));
  }
  async update(
    req: UpdateGrowthReferenceChartRequest
  ): Promise<AppServiceResponse<GrowthReferenceChartDto> | Message> {
  ...
  async delete(
    req: DeleteGrowthReferenceChartRequest
  ): Promise<AppServiceResponse<GrowthReferenceChartDto> | Message> {
