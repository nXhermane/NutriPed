...
export class GrowthReferenceTableService
  implements IGrowthReferenceTableService
{
  constructor(private readonly ucs: GrowthReferenceTableServiceUseCases) {}
  async create(
    req: CreateGrowthReferenceTableRequest
  ...
  async get(
    req: GetGrowthReferenceTableRequest
  ): Promise<AppServiceResponse<GrowthReferenceTableDto[]> | Message> {
    const res = await this.ucs.getUC.execute(req);
    if (res.isRight()) return { data: res.value.val };
    else return new Message("error", JSON.stringify((res.value as any)?.err));
  }
  async update(
    req: UpdateGrowthReferenceTableRequest
  ): Promise<AppServiceResponse<GrowthReferenceTableDto> | Message> {
  ...
  async delete(
    req: DeleteGrowthReferenceTableRequest
  ): Promise<AppServiceResponse<GrowthReferenceTableDto> | Message> {
