...
export class MakeClinicalSignDataInterpretationService
  implements IMakeClinicalSignDataInterpretationService
{
  constructor(
    private readonly ucs: MakeClinicalSignDataInterpretationUseCases
  ) {}
  async interpret(
    req: MakeClinicalSignDataInterpretationRequest
  ): Promise<
    AppServiceResponse<{ code: string; isPresent: boolean }[]> | Message
  > {
