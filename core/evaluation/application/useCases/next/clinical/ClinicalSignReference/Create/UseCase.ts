...
export class CreateClinicalSignReferenceUseCase implements UseCase<CreateClinicalSignReferenceRequest, CreateClinicalSignReferenceResponse> {
    constructor(private readonly repo: NextClinicalDomain.ClinicalSignReferenceRepository, private readonly idGenerator: GenerateUniqueId) { }
    async execute(request: CreateClinicalSignReference): Promise<CreateClinicalSignReferenceResponse> {
        try {
            const newId = this.idGenerator.generate().toValue()
            const clinicalRefResult = NextClinicalDomain.ClinicalSignReference.create(request, newId)
