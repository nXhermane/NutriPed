import { ApplicationMapper, handleError, left, Result, right, SystemCode, UseCase } from "@/core/shared";
...
export class GetNutritionalRiskFactorUseCase implements UseCase<GetNutritionalRiskFactorRequest, GetNutritionalRiskFactorResponse> {
    constructor(private readonly nutritionalRiskFactorRepo: NextClinicalDomain.NutritionalRiskFactorRepository, private readonly mapper: ApplicationMapper<NextClinicalDomain.NutritionalRiskFactor, NextClinicalDtos.NutritionalRiskFactorDto>) { }
    async execute(request: GetNutritionalRiskFactorRequest): Promise<GetNutritionalRiskFactorResponse> {
        try {
            ...
            if (request.clinicalSignCode && !request.id) {
                ...
                const risks = await this.nutritionalRiskFactorRepo.getByClinicalRefCode(codeRes.val)
            ...
            if (riskFactors.length == 0) {
                return left(Result.fail("Nutritional risk not found to provided ClinicalSignCode"))
            }
