...
export class CreateNutritionalRiskFactorUseCase implements UseCase<CreateNutritionalRiskFactorRequest, CreateNutritionalRiskFactorResponse> {
    constructor(private readonly idGenerator: GenerateUniqueId, private readonly nutritionalRiskFactor: NextClinicalDomain.NutritionalRiskFactorRepository) { }
    async execute(request: CreateNutritionalRiskFactorProps): Promise<CreateNutritionalRiskFactorResponse> {
        try {
            const newId = this.idGenerator.generate().toValue()
            const nutritionalRiskFactorRes = NextClinicalDomain.NutritionalRiskFactor.create(request, newId)
