...
export class NormalizeAndFillDefaultDataFieldResponseUseCase
  implements
    UseCase<
...
{
  constructor(
    private readonly normalizeDataFieldService: INormalizeDataFieldResponseService
  ) {}
  async execute(
    request: NormalizeAndFillDefaultDataFieldResponseRequest
  ): Promise<NormalizeAndFillDefaultDataFieldResponseResponse> {
    try {
      ...
      const normalizationResults =
        await this.normalizeDataFieldService.normalizeAndFillDefaults(
          fieldResponsesRes.val
        );
  ...
  private createAllDataFieldResponses(
    dataFieldResponseProps: CreateDataFieldResponse[]
  ): Result<DataFieldResponse[]> {
    try {
      const fieldResutls = dataFieldResponseProps.map(field =>
        DataFieldResponse.create(field)
      );
      ...
      if (combinedRes.isFailure) {
        return Result.fail(
          formatError(
            combinedRes,
            NormalizeAndFillDefaultDataFieldResponseUseCase.name
          )
        );
      }
