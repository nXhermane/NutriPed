...
export class UpdateAnthropometricMeasureUseCase
  implements
    UseCase<
    ...
    >
{
  ...
  async execute(
    request: UpdateAnthropometricMeasureRequest
  ): Promise<UpdateAnthropometricMeasureResponse> {
    try {
      const anthropometricMeasure =
        await this.anthropometricMeasureRepo.getById(request.id);
      const updatedRes = this.updateAnthropometricMeasure(
        anthropometricMeasure,
        request.data
      );
  ...
  private updateAnthropometricMeasure(
    measure: AnthropometricMeasure,
    data: UpdateAnthropometricMeasureRequest["data"]
  ): Result<unknown> {
    try {
      if (data.name) {
      ...
      if (data.unit && data.availableUnit) {
        ...
        measure.changeUnits({
          defaultUnit: defaultUnit.val,
          availableUnits: availableUnits.map(res => res.val),
        });
      }
