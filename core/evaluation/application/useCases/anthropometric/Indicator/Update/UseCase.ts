...
export class UpdateIndicatorUseCase
  implements UseCase<UpdateIndicatorRequest, UpdateIndicatorResponse>
{
  constructor(
    private readonly repo: IndicatorRepository,
    private readonly mapper: ApplicationMapper<Indicator, IndicatorDto>
  ) {}
  async execute(
  ...
  private updateIndicator(
    indicator: Indicator,
    data: UpdateIndicatorRequest["data"]
  ): Result<unknown> {
    try {
      if (data.name) {
      ...
      if (data.zScoreComputingStrategy) {
        indicator.changeZScoreComputingStrategyType(
          data.zScoreComputingStrategy
        );
      }
      if (data.availableRefCharts) {
        const availableRefCharts = data.availableRefCharts.map(
      ...
      if (data.interpretations) {
        const interpretations = data.interpretations.map(
