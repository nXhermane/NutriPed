...
export class CreateGrowthReferenceChartUseCase
  implements
    UseCase<
    ...
    >
{
  ...
  async execute(
    request: CreateGrowthReferenceChartRequest
  ): Promise<CreateGrowthReferenceChartResponse> {
    try {
      const growthReferenceChartId = this.idGenerator.generate().toValue();
      const growthReferenceChart = GrowthReferenceChart.create(
        request.data,
        growthReferenceChartId
      );
      if (growthReferenceChart.isFailure) return left(growthReferenceChart);
      const exist = await this.repo.exist(
        growthReferenceChart.val.getProps().code
      );
      if (exist)
        return left(
          Result.fail(
            `The growth reference chart with this code [${growthReferenceChart.val.getCode()}] already exist.`
          )
        );
