...
export class GetAppetiteTestUseCase
  implements UseCase<GetAppetiteTestRequest, GetAppetiteTestResponse>
{
  constructor(
    private readonly repo: AppetiteTestRefRepository,
  ...
  async execute(
    request: GetAppetiteTestRequest
  ): Promise<GetAppetiteTestResponse> {
    try {
      ...
      if (Boolean(appetiteTestId) === Boolean(appetiteTestCode)) {
        return left(
          Result.fail(
            "You must provide exactly one of appetiteTestId or appetiteTestCode"
          )
        );
      }
