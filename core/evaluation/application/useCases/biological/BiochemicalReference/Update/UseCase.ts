...
export class UpdateBiochemicalReferenceUseCase
  implements
    UseCase<
    ...
    >
{
  ...
  async execute(
    request: UpdateBiochemicalReferenceRequest
  ): Promise<UpdateBiochemicalReferenceResponse> {
    try {
      const biochemicalRef = await this.repo.getById(request.id);
      const updatedResult = this.updateBiochemicalRef(
        biochemicalRef,
        request.data
      );
  ...
  private updateBiochemicalRef(
    biochemicalRef: BiochemicalReference,
    data: UpdateBiochemicalReferenceRequest["data"]
  ): Result<ValueType> {
    try {
      if (data.name) {
