...
export class CorrectDiagnosticResultUseCase
  implements
    UseCase<CorrectDiagnosticResultRequest, CorrectDiagnosticResultResponse>
{
  constructor(
  ...
  async execute(
    request: CorrectDiagnosticResultRequest
  ): Promise<CorrectDiagnosticResultResponse> {
    try {
      const nutritionalDiagnostic =
        await this.nutritionalDiagnosticRepo.getById(
          request.nutritionalDiagnosticId
        );
      const prevNutritionalAssessmentResult =
        nutritionalDiagnostic.getDiagnosticResult();
      if (!prevNutritionalAssessmentResult)
        return left(
          Result.fail(
            "Please Generate an NutritionalDiagnostic before decide to modify it."
          )
        );
