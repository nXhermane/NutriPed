    ...
    >
{
  ...
  async execute(
    request: CreateNutritionalRiskFactorRequest
    ...
    try {
      ...
      const nutritionalRiskFactorRes = NutritionalRiskFactor.create(
        request.data,
        nutritionalRiskId
      );
  ...
  async checkIsValidClinicalCode(
    request: CreateNutritionalRiskFactorRequest
  ): Promise<Result<ValueType>> {
    try {
      const clinicalSignRefCodes = await this.clinicalRefRepo.getAllCode();
      const nutritionalRiskFactorCode = SystemCode.create(
        request.data.clinicalSignCode
      );
      if (nutritionalRiskFactorCode.isFailure) return nutritionalRiskFactorCode;
      const findResult = clinicalSignRefCodes.find(clinicalRefCode =>
        clinicalRefCode.equals(nutritionalRiskFactorCode.val)
      );
      if (!findResult)
        return Result.fail(
          "The code of nutritional risk factor is not correspond to any clinicalSignReference on a system."
        );
      return Result.ok();
    } catch (e: unknown) {
