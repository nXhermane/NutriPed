...
import { ClinicalNutritionalAnalysisResultDto } from "@/core/evaluation/application/dtos";

export class MakeClinicalAnalysisUseCase
  implements UseCase<MakeClinicalAnalysisRequest, MakeClinicalAnalysisResponse>
{
  constructor(
    private readonly clinicalDataValidatorService: IClinicalValidationService,
    private readonly clinicalSignAnalyserServer: IClinicalAnalysisService
  ) {}

  async execute(
    request: MakeClinicalAnalysisRequest
  ): Promise<MakeClinicalAnalysisResponse> {
    try {
      ...
      const clinicalDataValidationRes =
        await this.clinicalDataValidatorService.validate(clinicalDataRes.val);
      if (clinicalDataValidationRes.isFailure)
        return left(clinicalDataValidationRes);
      const clinicalAnalysisRes = await this.clinicalSignAnalyserServer.analyze(
        clinicalDataRes.val,
        evaluationContext
      );
      if (clinicalAnalysisRes.isFailure) return left(clinicalAnalysisRes);
      const clinicalAnalysisResults = clinicalAnalysisRes.val;
      const clinicalAnalysisResultDtos: ClinicalNutritionalAnalysisResultDto[] =
        clinicalAnalysisResults.map(clinicalAnalysis => {
