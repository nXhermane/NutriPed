...
@DomainEventMessage(
  "After Patient Deleted, On MedicalRecord Bc, Delete associated Medical Record",
  true
)
export class AfterPatientDeletedMedicalRecordHandler extends EventHandler<
  PatientDeletedData,
  PatientDeletedEvent
> {
  constructor(
  ...
  private async onPatientDeletedEvent(eventData: PatientDeletedData) {
    const deleteMedicalRecordRes = await this.deleteUseCase.execute({
      patientOrMedicalRecordId: eventData.id,
    });
    if (deleteMedicalRecordRes.isLeft())
      throw new EventHandlerExecutionFailed(
        JSON.stringify((deleteMedicalRecordRes.value as any)?.err)
      );
  }
}
