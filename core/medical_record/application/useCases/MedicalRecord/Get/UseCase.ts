...
export class GetMedicalRecordUseCase
  implements UseCase<GetMedicalRecordRequest, GetMedicalRecordResponse>
{
  constructor(
    private readonly repo: MedicalRecordRepository,
    private readonly mapper: ApplicationMapper<MedicalRecord, MedicalRecordDto>
  ) {}
  async execute(
    request: GetMedicalRecordRequest
  ): Promise<GetMedicalRecordResponse> {
    try {
      const medical_record = await this.repo.getByPatientIdOrID(
