...
export class NormalizeDataFieldResponseACl
  implements INormalizeDataFieldResponseAcl
{
  constructor(
  ...
  > {
    try {
      ...
      const normalizationResult =
        await this.normalizeDataService.normalizeAndFillDefault({
          data: request,
        });
      if ("data" in normalizationResult) {
        const normalizedFields = normalizationResult.data;
        return Result.ok(
          Object.entries(normalizationResult).map(fieldEntries => ({
            code: fieldEntries[0] as DATA_FIELD_CODE_TYPE,
            value: fieldEntries[1] as number | string,
          }))
        );
      }
