...
import { AggregateID, ArgumentNotProvidedException, DomainDate, Entity, EntityPropsBaseType, formatError, Guard, handleError, Result, SystemCode } from "@/core/shared";
...
export class DataFieldResponse extends Entity<IDataFieldResponse> {
    ...
    public validate(): void {
        ...
        if (Guard.isEmpty(this.props.data).succeeded) {
            throw new ArgumentNotProvidedException("The data of data field response can't be empty.")
        }
        this._isValid = true
    }
    static create(createProps: CreateDataFieldResponse, id: AggregateID): Result<DataFieldResponse> {
        try {
            ...
            return Result.ok(new DataFieldResponse({
                id, props: {
                    code: codeRes.val,
