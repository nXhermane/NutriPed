...
export class AppetiteTestRecord extends Entity<IAppetiteTestRecord> {
    ...
    public validate(): void {
        ...
        if (this.props.productType === APPETITE_TEST_PRODUCT_TYPE.IN_SACHET) {
            if (Guard.isEmpty((this.props.amount as TakenAmountInSachet)?.fraction).succeeded) {
                throw new ArgumentInvalidException(
                    "Please if the taken product is in sachet provide a product taken fraction in available options."
                );
            }
            else if (!Object.values(APPETITE_TEST_SACHET_FRACTION_PARTITION).includes((this.props.amount as TakenAmountInSachet)?.fraction)) {
        ...
        if (this.props.productType === APPETITE_TEST_PRODUCT_TYPE.IN_POT) {
            if (Guard.isEmpty((this.props.amount as TakenAmountOfPot).quantity).succeeded) {
                throw new ArgumentInvalidException(
                    "Please if the taken product is in pot, provide the taken amount as a number"
                );
            }
            else if (Guard.isNegative((this.props.amount as TakenAmountOfPot).quantity).succeeded) {
    ...
    static create(createProps: CreateAppetiteTestRecord, id: AggregateID): Result<AppetiteTestRecord> {
        try {
            ...
            return Result.ok(new AppetiteTestRecord({
                id, props: {
